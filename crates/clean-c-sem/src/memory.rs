...
impl SharedProvenanceModel for Pointer {
    type AllocId = BlockId;
...
impl Memory {
    ...
    pub fn load_bytes(&self, ptr: Pointer, size: usize) -> UBResult<Vec<u8>> {
        ...
        let block = self
            .blocks
            .get(&ptr.block)
            .expect("invariant: can_read verified the block exists");
    ...
    pub fn store_bytes(&mut self, ptr: Pointer, bytes: &[u8]) -> UBResult<()> {
        ...
        let block = self
            .blocks
            .get_mut(&ptr.block)
            .expect("invariant: can_write verified the block exists");
        let start = ptr.offset as usize;
...
mod tests {
    ...
    fn test_out_of_bounds() {
        ...
        // In bounds
        mem.load_bytes(ptr, 10)
            .expect("load within allocation bounds should succeed");
    ...
    fn test_store_pointer_overflow_block_id() {
        ...
        assert!(
            matches!(result, Err(UBKind::InvalidPointer)),
            "expected InvalidPointer for oversized block ID, got: {result:?}"
        );
    }
    ...
    fn test_store_pointer_overflow_offset() {
        ...
        let result = mem.store_ptr(ptr, target);
        assert!(
            matches!(result, Err(UBKind::PointerOverflow)),
            "expected PointerOverflow for oversized offset, got: {result:?}"
        );
    }
    ...
    fn test_free_null() {
        let mut mem = Memory::new();
        // free(NULL) is a no-op
        mem.free(Pointer::null())
            .expect("free(NULL) should be a no-op");
    }
    ...
    #[test]
    fn test_shared_memory_trait_adapter() {
        let mut mem = Memory::new();
        let ptr = SharedMemoryModel::alloc(&mut mem, 4, 4).expect("allocation should succeed");

        assert_eq!(ptr.alloc_id(), BlockId(1));
        assert_eq!(ptr.offset_bytes(), 0);
        assert_eq!(SharedMemoryModel::null_ptr(&mem), Pointer::null());

        SharedMemoryModel::write_bytes(&mut mem, ptr, &[0xAA, 0xBB, 0xCC, 0xDD])
            .expect("shared trait write should succeed");
        assert_eq!(
            SharedMemoryModel::read_bytes(&mem, ptr, 4).expect("shared trait read should succeed"),
            vec![0xAA, 0xBB, 0xCC, 0xDD]
        );

        SharedMemoryModel::dealloc(&mut mem, ptr).expect("shared trait free should succeed");
        assert!(!SharedMemoryModel::is_valid(&mem, ptr));
    }
}
