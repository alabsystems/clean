...
//! KKL real/cbrt layer (ported from the sqrt layer; cbrtDyadicApprox mirrors dyadicApprox) — `IsCauchy (dyadicApproxNN x)` (Stage B3, sqrt run #4,
//! rung 6 CAPSTONE).
//!
...
//!
//! - `Rat.cbrtDyadicApproxNN : Rat → Nat → NNRat`   (reducible Def)
//! - `NNReal.cbrtDyadicApprox_isCauchy : ∀ x, NNReal.IsCauchy (Rat.cbrtDyadicApproxNN x)`
//!
...
pub(crate) struct CbrtIsCauchyConsts {
    nat: Expr,
    nat_zero: Expr,
    nat_succ: Expr,
    nat_pow: Expr,
...
impl CbrtIsCauchyConsts {
    pub(crate) fn new() -> Self {
        ...
        Self {
            ...
            rat_zero_le_dyadic_approx: k("Rat.zero_le_cbrtDyadicApprox"),
            rat_dyadic_approxnn: k("Rat.cbrtDyadicApproxNN"),
            nnreal_is_cauchy: k("NNReal.IsCauchy"),
            rat_exists_inv_two_pow_lt: k("Rat.exists_inv_two_pow_lt"),
            rat_dyadic_mono: k("Rat.cbrtDyadicApprox_mono"),
            rat_dyadic_le_add_inv_of_le: k("Rat.cbrtDyadicApprox_le_add_inv_of_le"),
...
impl Environment {
    /// Register `Rat.cbrtDyadicApproxNN` + `NNReal.cbrtDyadicApprox_isCauchy`.
    /// Idempotent; axiom-free.
    pub fn init_algebra_nnreal_cbrt_iscauchy(&mut self) -> Result<(), EnvError> {
        self.init_eq()?;
        self.init_and()?;
        self.init_exists()?;
        self.init_nat()?;
        self.init_algebra_nnreal_cauchy()?; // NNReal.IsCauchy, NNRat
        self.init_algebra_nnreal_cbrt_seq()?; // dyadicApprox, zero_le_dyadicApprox
        self.init_algebra_nnreal_cbrt_cauchy_le()?; // mono, le_add_inv_of_le
        ...
        let c = CbrtIsCauchyConsts::new();
        self.register_cbrt_dyadic_approxnn(&c)?;
        self.register_cbrt_dyadic_approx_iscauchy(&c)?;
        Ok(())
    }

    /// `Rat.cbrtDyadicApproxNN : Rat → Nat → NNRat`
    ///   `:= fun x n => NNRat.ofRat (dyadicApprox x n) (zero_le_dyadicApprox x n)`.
    fn register_cbrt_dyadic_approxnn(&mut self, c: &CbrtIsCauchyConsts) -> Result<(), EnvError> {
        let name = Name::from_string("Rat.cbrtDyadicApproxNN");
    ...
    /// `NNReal.cbrtDyadicApprox_isCauchy : ∀ x, NNReal.IsCauchy (Rat.cbrtDyadicApproxNN x)`.
    fn register_cbrt_dyadic_approx_iscauchy(
        &mut self,
        c: &CbrtIsCauchyConsts,
    ) -> Result<(), EnvError> {
        let name = Name::from_string("NNReal.cbrtDyadicApprox_isCauchy");
        if self.get_const(&name).is_some() {
            return Ok(());
        }
        let ty = {
...
mod tests {
    ...
    const DEFS: &[&str] = &["Rat.cbrtDyadicApproxNN"];
    const THEOREMS: &[&str] = &["NNReal.cbrtDyadicApprox_isCauchy"];

    fn env() -> Environment {
        let mut env = Environment::with_prelude();
        env.init_algebra_nnreal_cbrt_iscauchy()
            .expect("init_algebra_nnreal_cbrt_iscauchy");
        env.init_algebra_nnreal_cbrt_iscauchy().expect("idempotent");
        env
    }
