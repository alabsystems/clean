...
//! KKL real/sqrt layer — the dyadic approximation is NONDECREASING
...
//! `Rat.dyadicNum_two_mul_le_succ` (`2k_n ≤ k_{n+1}`) and the LANDED inv step
//! factorization `Rat.inv_two_pow_succ`.
//!
//! # What this module registers (axiom-free, kernel-checked)
//!
//! - `Rat.dyadicApprox_le_succ : ∀ x n,
//!       Rat.le (Rat.dyadicApprox x n) (Rat.dyadicApprox x (Nat.succ n))`.
//!
...
pub(crate) struct MonoStepConsts {
    nat: Expr,
    nat_zero: Expr,
    nat_succ: Expr,
    nat_mul: Expr,
...
impl MonoStepConsts {
    pub(crate) fn new() -> Self {
        ...
        Self {
            ...
            rat_dyadic_num_two_mul_le_succ: k("Rat.dyadicNum_two_mul_le_succ"),
...
impl Environment {
    /// Register `Rat.dyadicApprox_le_succ`. Idempotent; axiom-free.
    pub fn init_algebra_nnreal_sqrt_cauchy_mono(&mut self) -> Result<(), EnvError> {
        self.init_eq()?;
        self.init_and()?;
        self.init_iff()?;
        self.init_nat()?;
        // Rat.dyadicApprox + Rat.dyadicNum + Rat.ofNat.
        self.init_algebra_nnreal_sqrt_seq()?;
        // Rat.dyadicNum_two_mul_le_succ (digit lower bound).
        self.init_algebra_nnreal_sqrt_mono()?;
        ...
        let c = MonoStepConsts::new();
        self.register_dyadic_approx_le_succ(&c)?;
        Ok(())
    }

    fn register_dyadic_approx_le_succ(&mut self, c: &MonoStepConsts) -> Result<(), EnvError> {
        let name = Name::from_string("Rat.dyadicApprox_le_succ");
...
mod tests {
    ...
    const THEOREMS: &[&str] = &["Rat.dyadicApprox_le_succ"];

    fn env() -> Environment {
        let mut env = Environment::with_prelude();
        env.init_algebra_nnreal_sqrt_cauchy_mono()
            .expect("init_algebra_nnreal_sqrt_cauchy_mono");
        env.init_algebra_nnreal_sqrt_cauchy_mono()
            .expect("idempotent");
        env
    }
