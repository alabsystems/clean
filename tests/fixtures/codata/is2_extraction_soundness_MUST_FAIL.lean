-- NEGATIVE CONTROL for is2_extraction_soundness.lean. MUST FAIL TO CHECK.
--
-- Identical to that file except the emitted step is WRONG: `succ acc` where
-- the source doubles. If the observational theorem still went through, it
-- would be insensitive to the very term it is about -- i.e. vacuous. The
-- refusal here is what makes the positive file's theorem mean something.
--
-- Rank-7 B7 negative control.
--
