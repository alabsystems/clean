...
//! KKL real/sqrt layer — the TELESCOPING bound `a_{n+d} ≤ a_n + inv(2^n)`
...
//!
//! - `Rat.dyadicApprox_le_add_inv : ∀ x n d,
//!       Rat.le (Rat.dyadicApprox x (Nat.add n d))
//!              (Rat.add (Rat.dyadicApprox x n) (inv (ofNat 2^n)))`.
//!
...
impl TeleConsts {
    pub(crate) fn new() -> Self {
        ...
        Self {
            ...
            rat_dyadic_approx_succ_le: k("Rat.dyadicApprox_succ_le"),
...
impl Environment {
    /// Register `Rat.dyadicApprox_le_add_inv`. Idempotent; axiom-free.
    pub fn init_algebra_nnreal_sqrt_cauchy_tele(&mut self) -> Result<(), EnvError> {
        self.init_eq()?;
        self.init_and()?;
        self.init_iff()?;
        self.init_nat()?;
        ...
        self.init_algebra_nnreal_sqrt_cauchy_step()?; // dyadicApprox_succ_le, zero_lt_inv_two_pow
        ...
        let c = TeleConsts::new();
        self.register_dyadic_approx_le_add_inv(&c)?;
        Ok(())
    }

    fn register_dyadic_approx_le_add_inv(&mut self, c: &TeleConsts) -> Result<(), EnvError> {
        let name = Name::from_string("Rat.dyadicApprox_le_add_inv");
...
mod tests {
    ...
    const THEOREMS: &[&str] = &["Rat.dyadicApprox_le_add_inv"];

    fn env() -> Environment {
        let mut env = Environment::with_prelude();
        env.init_algebra_nnreal_sqrt_cauchy_tele()
            .expect("init_algebra_nnreal_sqrt_cauchy_tele");
        env.init_algebra_nnreal_sqrt_cauchy_tele()
            .expect("idempotent");
        env
    }
