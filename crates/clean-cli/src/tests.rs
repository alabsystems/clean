...
use super::cli_args::*;
use super::cmd_commit::{commit_ipa, commit_kzg, commit_verify};
use super::cmd_core::{check_file, eval_expr, verify_c_file};
use super::cmd_fold::{fold_compress, fold_extend, fold_info, fold_start, fold_verify};
use serde_json::Value;
use std::fs;
use std::path::PathBuf;
use {
    clean_c_sem::cli::VerifyCArgs,
    clean_elab::cli::EvalArgs,
    clean_fold::cli::FoldCommands,
    clean_fold::commit::cli::CommitCommands,
    clean_kernel::cli::{CheckArgs, PreludeMode},
    clean_server::cli::ServerArgs,
};
...
";
    ...
    let dir = tempfile::tempdir().expect("tempdir should be created");
    let path = dir.path().join("simple.clean");
    fs::write(&path, code).expect("write should succeed");

    check_file(&path, false, false, PreludeMode::Builtin)
        .expect("Simple definition should type check");
}

#[test]
fn check_file_nonexistent_fails() {
    let path = PathBuf::from("/nonexistent/path/to/file.clean");
...
";
    ...
    let dir = tempfile::tempdir().expect("tempdir should be created");
    let path = dir.path().join("const.clean");
    fs::write(&path, code).expect("write should succeed");

    check_file(&path, true, false, PreludeMode::Builtin).expect("Verbose mode should work");
}
...
fn check_file_empty_file() {
    ...
    let dir = tempfile::tempdir().expect("tempdir should be created");
    let path = dir.path().join("empty.clean");
    fs::write(&path, code).expect("write should succeed");

    // Empty file should succeed (0 declarations)
    check_file(&path, false, false, PreludeMode::Builtin).expect("Empty file should be valid");
}
...
";
    ...
    let dir = tempfile::tempdir().expect("tempdir should be created");
    let path = dir.path().join("multi.clean");
    fs::write(&path, code).expect("write should succeed");
...
fn cli_parse_check_command() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "check", "test.clean"]).unwrap();
    match cli.command {
        Commands::Check(CheckArgs { file, verbose, .. }) => {
            assert_eq!(file, PathBuf::from("test.clean"));
            assert!(!verbose);
        }
...
fn cli_parse_check_verbose() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "check", "-v", "test.clean"]).unwrap();
    match cli.command {
        Commands::Check(CheckArgs { verbose, .. }) => {
            assert!(verbose);
        }
        _ => panic!("Expected Check command"),
    }
...
#[test]
fn cli_parse_check_max_cache_entries_default_none() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "check", "test.clean"]).unwrap();
    match cli.command {
        Commands::Check(CheckArgs {
            ...
            assert_eq!(
                ...
                "absent flag must leave the kernel default in place"
            );
        }
        _ => panic!("Expected Check command"),
    }
...
#[test]
fn cli_parse_check_max_cache_entries_explicit() {
    use clap::Parser;
    ...
    match cli.command {
        Commands::Check(CheckArgs {
            ...
            assert_eq!(max_cache_entries, Some(1_000_000));
        }
        _ => panic!("Expected Check command"),
    }
}
...
#[test]
fn cli_parse_check_max_cache_entries_zero_means_unbounded() {
    use clap::Parser;
    let cli =
        Cli::try_parse_from(["clean", "check", "--max-cache-entries", "0", "test.clean"]).unwrap();
    match cli.command {
        Commands::Check(CheckArgs {
            max_cache_entries, ..
        }) => {
            // `0` parses through to the dispatcher, which maps it to an
            // effectively-unbounded cap (no eviction).
            assert_eq!(max_cache_entries, Some(0));
        }
        _ => panic!("Expected Check command"),
    }
}
        ...
        Commands::Lake(clean_lake::cli::LakeArgs { command, dir }) => {
            ...
            match command {
                LakeCommands::Run {
                    ...
                    assert_eq!(args, ["alpha", "--literal-flag"]);
                    assert!(verbose);
                    assert_eq!(jobs, 4);
                }
...
#[test]
fn cli_parse_lake_run_forwards_flag_like_args_after_target() {
    use clap::Parser;
    let cli = Cli::try_parse_from([
        "clean",
        "lake",
...
#[test]
fn cli_parse_lake_env_with_command_args() {
    use clap::Parser;
    let cli = Cli::try_parse_from([
        "clean",
        "lake",
...
fn cli_parse_eval_command() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "eval", "Type"]).unwrap();
    match cli.command {
        Commands::Eval(EvalArgs { expr, verbose }) => {
            assert_eq!(expr, "Type");
            assert!(!verbose);
        }
...
fn cli_parse_server_default() {
    use clap::Parser;
    fn parse_server(args: &[&str]) -> (u16, bool, bool, bool, bool, Option<PathBuf>) {
        ...
        match cli.command {
            Commands::Server(ServerArgs {
                ...
                stdlib,
                theorem_index,
            }) => (port, no_gpu, websocket, init, stdlib, theorem_index),
            _ => panic!("Expected Server command"),
        }
    }

    assert_eq!(
        parse_server(&["clean", "server"]),
        (8080, false, false, false, false, None)
    );
    assert_eq!(
        parse_server(&["clean", "server", "--port", "9000"]),
        (9000, false, false, false, false, None)
    );
    assert_eq!(
        parse_server(&["clean", "server", "--no-gpu"]),
        (8080, true, false, false, false, None)
    );
    assert_eq!(
        parse_server(&["clean", "server", "--websocket"]),
        (8080, false, true, false, false, None)
    );
    assert_eq!(
        parse_server(&["clean", "server", "--init"]),
        (8080, false, false, true, false, None)
    );
    assert_eq!(
        parse_server(&["clean", "server", "--stdlib"]),
        (8080, false, false, false, true, None)
    );
    assert_eq!(
        parse_server(&["clean", "server", "--theorem-index", "index.json"]),
        (
            8080,
            false,
            false,
...
fn cli_parse_verify_c_command() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "verify-c", "test.c"]).unwrap();
    match cli.command {
        Commands::VerifyC(VerifyCArgs {
            file,
            fail_unknown,
            verbose,
        }) => {
...
fn cli_parse_verify_c_fail_unknown() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "verify-c", "--fail-unknown", "test.c"]).unwrap();
    match cli.command {
        Commands::VerifyC(VerifyCArgs { fail_unknown, .. }) => {
            assert!(fail_unknown);
        }
        _ => panic!("Expected VerifyC command"),
    }
...
fn cli_parse_repl_command() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "repl"]).unwrap();
...
fn cli_missing_subcommand_fails() {
    use clap::Parser;
    let result = Cli::try_parse_from(["clean"]);
...
fn cli_unknown_subcommand_fails() {
    use clap::Parser;
    let result = Cli::try_parse_from(["clean", "unknown"]);
...
fn cli_parse_lake_resolve() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "resolve", "-v"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
...
fn cli_parse_lake_resolve_dry_run() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "resolve", "--dry-run"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
            LakeCommands::Resolve { verbose, dry_run } => {
                assert!(!verbose);
...
fn cli_parse_lake_exe() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "exe", "myapp"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
            LakeCommands::Exe {
                name,
...
fn cli_parse_lake_exe_with_args() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "exe", "myapp", "arg1", "arg2"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
...
fn cli_parse_lake_test() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "test"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
            LakeCommands::Test {
                target,
                ...
                assert!(target.is_none(), "default test target should be None");
                assert!(args.is_empty(), "default test args should be empty");
                assert!(!verbose);
                assert_eq!(jobs, 0);
            }
...
fn cli_parse_lake_test_with_target() {
    use clap::Parser;
    let cli =
        Cli::try_parse_from(["clean", "lake", "test", "-v", "-j", "4", "unit_tests"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
            LakeCommands::Test {
                target,
                ...
                assert!(args.is_empty(), "targeted test args should be empty");
                assert!(verbose);
                assert_eq!(jobs, 4);
            }
            _ => panic!("Expected Lake test command"),
...
#[test]
fn cli_parse_lake_test_with_forwarded_args() {
    use clap::Parser;
    let cli = Cli::try_parse_from([
        "clean",
        "lake",
...
fn cli_parse_lake_script_list() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "script", "list"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
...
fn cli_parse_lake_script_run() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "script", "run", "build_docs"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
...
fn cli_parse_lake_script_doc() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "script", "doc", "build_docs"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
...
fn cli_parse_lake_cache_get() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "cache", "get"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
...
fn cli_parse_lake_cache_put_verbose() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "cache", "put", "-v"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, .. }) => match command {
...
fn fold_start_creates_ivc_proof() {
    ...
    let proof: Value = serde_json::from_str(&content).expect("output should be valid JSON");
    assert!(num(&proof, "step") > 0);
    assert!(num(&proof, "num_constraints") > 0);
}
...
fn commit_kzg_creates_commitment() {
    ...
    let content = fs::read_to_string(&output_path).expect("read should succeed");
    let commitment: Value = serde_json::from_str(&content).expect("output should be valid JSON");

    assert_eq!(text(&commitment, "scheme"), "KZG");
    assert_eq!(num(&commitment, "max_degree"), 8);
    assert!(!text(&commitment, "commitment").is_empty());
}
...
fn commit_ipa_creates_commitment() {
    ...
    assert_eq!(text(&commitment, "scheme"), "IPA");
    assert_eq!(num(&commitment, "max_degree"), 8);
}
    ...
    match cli.command {
        Commands::Fold { command } => match command {
            FoldCommands::Extend {
                ...
                assert_eq!(cert, PathBuf::from("cert.json"));
                assert!(output.is_none(), "no --output flag should mean None");
                assert!(!verbose);
            }
...
fn cli_parse_fold_verify() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "fold", "verify", "ivc.json"]).unwrap();
    match cli.command {
        Commands::Fold { command } => match command {
...
fn cli_parse_fold_compress() {
    use clap::Parser;
    let cli = Cli::try_parse_from([
        "clean", "fold", "compress", "-i", "ivc.json", "-o", "out.json",
    ])
    .unwrap();
    match cli.command {
        Commands::Fold { command } => match command {
...
fn cli_parse_fold_info() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "fold", "info", "ivc.json"]).unwrap();
    match cli.command {
        Commands::Fold { command } => match command {
...
fn cli_parse_lake_dir_long_option() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "--dir", "/path/to/project", "build"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, dir }) => {
...
fn cli_parse_lake_dir_short_option() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "-d", "/tmp/myproject", "env"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, dir }) => {
...
fn cli_parse_lake_dir_relative_path() {
    use clap::Parser;
    let cli = Cli::try_parse_from(["clean", "lake", "-d", "my-project", "clean"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, dir }) => {
...
fn cli_parse_lake_dir_with_subcommand_options() {
    use clap::Parser;
    let cli =
        Cli::try_parse_from(["clean", "lake", "-d", "/project", "build", "-v", "-f"]).unwrap();
    match cli.command {
        Commands::Lake(clean_lake::cli::LakeArgs { command, dir }) => {
...
// ========== clean kernel verify-constructive-claims (#3510) ==========

/// `clean kernel verify-constructive-claims --conjecture C008` must route
/// through `Commands::Kernel` and bind `conjecture = "C008"`,
/// `allow_empty = false`.
#[test]
fn cli_parse_kernel_verify_constructive_claims_basic() {
    use clap::Parser;
    ...
    .expect("basic `verify-constructive-claims --conjecture C008` must parse");
    ...
    match cli.command {
        Commands::Kernel {
            ...
            assert_eq!(conjecture, "C008");
            assert!(!allow_empty, "--allow-empty absent must be `false`");
        }
        other => panic!(
            "expected Commands::Kernel(VerifyConstructiveClaims), got: {:?}",
            std::any::type_name_of_val(&other)
        ),
    }
}

/// `--allow-empty` must bind to the second field of the variant. This
/// exercises the optional-flag surface the Python audit gate depends on when
/// a conjecture's namespace is empty.
#[test]
fn cli_parse_kernel_verify_constructive_claims_allow_empty() {
    use clap::Parser;
    use clean_kernel::cli::KernelCommands;
    ...
    .expect("`--conjecture C010 --allow-empty` must parse");
    ...
    match cli.command {
        Commands::Kernel {
            ...
            assert_eq!(conjecture, "C010");
            assert!(allow_empty, "`--allow-empty` present must bind `true`");
        }
        _ => panic!("expected Commands::Kernel(VerifyConstructiveClaims)"),
    }
}

/// Missing the required `--conjecture <ID>` flag must be a clap-level
/// usage error (the legacy binary exited 2 on the same input). Without
/// this test, a future refactor that accidentally gives `--conjecture` a
/// default value would silently pass empty audits.
#[test]
fn cli_parse_kernel_verify_constructive_claims_missing_conjecture_errors() {
    use clap::Parser;

    let result = Cli::try_parse_from(["clean", "kernel", "verify-constructive-claims"]);
    assert!(
        result.is_err(),
        "`verify-constructive-claims` without --conjecture must fail parse",
    );
}

// ========== MacroCtx persistence (two-language design §4.2 hardening) ==========

/// Single-file `clean check` must thread `FileContext` through elaboration so
/// a `notation`/custom-operator declaration registered by one declaration is
/// still in scope for the NEXT declaration — the "MacroCtx drop" fix. Before
/// the fix this file's second/third declarations failed with UnknownIdent /
/// parse errors while `clean project check` accepted them.
#[test]
fn check_file_notation_persists_across_declarations() {
    let code = r#"
def Always (p : Nat -> Prop) : Prop := forall n, p n

prefix:100 "□" => Always

notation "alwaysTriv" => Always (fun n => True)

def use_prefix_notation : Prop := □ (fun n => True)

def use_term_notation : Prop := alwaysTriv

theorem notation_is_definitional : use_prefix_notation = Always (fun n => True) := rfl
"#;

    let dir = tempfile::tempdir().expect("tempdir should be created");
    let path = dir.path().join("notation_persist.clean");
    fs::write(&path, code).expect("write should succeed");

    check_file(&path, false, false, PreludeMode::Builtin)
        .expect("notation declared in one declaration must persist to later declarations");
}
