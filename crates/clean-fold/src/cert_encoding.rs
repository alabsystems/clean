...
use clean_kernel::{BinderInfo, Expr, Level, Literal, Name};
...
impl CertR1CSEncoder {
    ...
    fn encode_cert(&mut self, cert: &ProofCert) -> Result<usize, FoldError> {
        match cert {
            ...
            ProofCert::FVar { id, type_ } => Ok(self.encode_fvar(id.as_u64(), type_)),
            ProofCert::Const {
                name,
                levels,
                type_,
            ...
            ProofCert::CubicalCoe {
                ...
                base_cert,
                result_type,
            } => self.encode_cubical_coe(ty_cert, r_cert, s_cert, base_cert, result_type),

            // ════════════════════════════════════════════════════════════════════════
            // ZFC/Set-theoretic mode certificates
            // ════════════════════════════════════════════════════════════════════════
            ProofCert::ZFCSet { kind, result_type } => self.encode_zfc_set(kind, result_type),
    ...
    fn encode_def_eq(
        ...
        expected_type: &Expr,
        actual_type: &Expr,
        eq_steps: &[clean_kernel::cert::DefEqStep],
    ) -> Result<usize, FoldError> {
    ...
    fn encode_cubical_coe(
        ...
        let tag_var = self.alloc(Scalar::from(tags::CUBICAL_COE));
        self.constrain_const(
            self.builder.var_idx(tag_var),
            Scalar::from(tags::CUBICAL_COE),
        );
        let _ty_cert_var = self.encode_cert(ty_cert)?;
        let _r_cert_var = self.encode_cert(r_cert)?;
        let _s_cert_var = self.encode_cert(s_cert)?;
        let _base_cert_var = self.encode_cert(base_cert)?;
        let _result_type_var = self.encode_expr(result_type);
        let node_hash = hash_cubical_coe(ty_cert, r_cert, s_cert, base_cert, result_type);
        Ok(self.alloc(node_hash))
    }
    ...
    fn encode_zfc_set_kind(&mut self, kind: &ZFCSetCertKind) -> Result<usize, FoldError> {
        match kind {
            ...
            ZFCSetCertKind::Pair(cert1, cert2) => {
                ...
                self.constrain_const(self.builder.var_idx(tag_var), Scalar::from(tags::zfc::PAIR));
...
fn hash_cubical_coe(
    ...
    let mut hasher = DefaultHasher::new();
    tags::CUBICAL_COE.hash(&mut hasher);
    format!("{ty_cert:?}").hash(&mut hasher);
    format!("{r_cert:?}").hash(&mut hasher);
    format!("{s_cert:?}").hash(&mut hasher);
    format!("{base_cert:?}").hash(&mut hasher);
    format!("{result_type:?}").hash(&mut hasher);
    Scalar::from(hasher.finish())
}
...
mod tests {
    ...
    fn test_encode_app() {
        ...
        let cert = ProofCert::App {
            ...
            fn_type: Box::new(Expr::from_kind(ExprKind::Sort(Level::Zero))),
    ...
    fn test_encode_lit_nat() {
        let cert = ProofCert::Lit {
            ...
            type_: Box::new(Expr::from_kind(ExprKind::Const(
                "Nat".parse().unwrap(),
                smallvec![],
            ))),
        };
    ...
    fn test_encode_cubical_path_lam() {
        ...
        let cert = ProofCert::CubicalPathLam {
            body_cert: Box::new(body_cert),
            body_type: Box::new(Expr::from_kind(ExprKind::CubicalInterval)),
            result_type: Box::new(Expr::from_kind(ExprKind::Sort(Level::Zero))),
        };
    ...
    fn test_encode_cubical_path_app() {
        ...
        let cert = ProofCert::CubicalPathApp {
            ...
            arg_cert: Box::new(arg_cert),
            path_type: Box::new(Expr::from_kind(ExprKind::Sort(Level::Zero))),
            result_type: Box::new(Expr::from_kind(ExprKind::Sort(Level::Zero))),
        };
