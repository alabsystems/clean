...
//! Constructive proof of `Fin.sum_sub`.
//!
//! The proof is induction over the faithful `Fin.sum` carrier. The successor
//! step uses the checked `Rat.sub_add_sub` normalization:
//! `(A - B) + (a - b) = (A + a) - (B + b)`.
...
struct FinSumSubConsts {
    ...
    congr_arg: Expr,
    rat_sub_self: Expr,
    rat_sub_add_sub: Expr,
}

impl FinSumSubConsts {
    fn new() -> Self {
        ...
        Self {
            ...
            rat_sub_add_sub: Expr::const_(Name::from_string("Rat.sub_add_sub"), vec![]),
        }
    }
    ...
    fn sub_rat(&self, lhs: Expr, rhs: Expr) -> Expr {
        Expr::app(Expr::app(self.base.rat_sub.clone(), lhs), rhs)
    }
...
fn pointwise_sub(c: &FinSumSubConsts, parent: &EnvDeclBuilder, n: Expr, f: Expr, g: Expr) -> Expr {
    let fin_n = Expr::app(c.base.fin.clone(), n);
    let mut b = EnvDeclBuilder::child_of(parent);
    let (i_id, i) = b.fresh_local(fin_n.clone());
    let body = c.sub_rat(Expr::app(f, i.clone()), Expr::app(g, i));
    let lam = b.mk_lam(i_id, BinderInfo::Default, fin_n, body);
    b.finish_child(lam)
}

fn cast_succ_fn(c: &FinSumSubConsts, parent: &EnvDeclBuilder, k: Expr, f: Expr) -> Expr {
...
fn add_right_fn(c: &FinSumSubConsts, parent: &EnvDeclBuilder, right: Expr) -> Expr {
    let mut b = EnvDeclBuilder::child_of(parent);
    let (x_id, x) = b.fresh_local(c.base.rat.clone());
    let body = Expr::app(Expr::app(c.base.rat_add.clone(), x), right);
    let lam = b.mk_lam(x_id, BinderInfo::Default, c.base.rat.clone(), body);
    b.finish_child(lam)
}

fn congr_rat(c: &FinSumSubConsts, alpha: Expr, a: Expr, b: Expr, f: Expr, h: Expr) -> Expr {
    Expr::apps(c.congr_arg.clone(), [alpha, c.base.rat.clone(), a, b, f, h])
}

fn eq_trans(c: &FinSumSubConsts, a: Expr, b: Expr, d: Expr, h1: Expr, h2: Expr) -> Expr {
    Expr::apps(c.eq_trans.clone(), [c.base.rat.clone(), a, b, d, h1, h2])
}

fn eq_symm(c: &FinSumSubConsts, a: Expr, b: Expr, h: Expr) -> Expr {
    Expr::apps(c.eq_symm.clone(), [c.base.rat.clone(), a, b, h])
}

fn build_type(c: &FinSumSubConsts) -> Expr {
    ...
    let lhs = c.sum(
        n.clone(),
        pointwise_sub(c, &b, n.clone(), f.clone(), g.clone()),
    );
    let rhs = c.sub_rat(c.sum(n.clone(), f), c.sum(n, g));
...
fn build_motive(c: &FinSumSubConsts) -> Expr {
    let mut b = EnvDeclBuilder::new();
    ...
    let lhs = c.sum(
        k.clone(),
        pointwise_sub(c, &b, k.clone(), f.clone(), g.clone()),
    );
    let rhs = c.sub_rat(c.sum(k.clone(), f), c.sum(k, g));
...
fn build_base(c: &FinSumSubConsts) -> Expr {
    let mut b = EnvDeclBuilder::new();
    let f_type = c.fin_to_rat(c.nat_zero.clone());
    let (f_id, _f) = b.fresh_local(f_type.clone());
    let (g_id, _g) = b.fresh_local(f_type.clone());
    let sub_zero_zero = c.sub_rat(c.base.rat_zero.clone(), c.base.rat_zero.clone());
    let h = Expr::app(c.rat_sub_self.clone(), c.base.rat_zero.clone());
    let proof = eq_symm(c, c.base.rat_zero.clone(), sub_zero_zero, h);
    let val = b.mk_lam(g_id, BinderInfo::Default, f_type.clone(), proof);
    let val = b.mk_lam(f_id, BinderInfo::Default, f_type, val);
    b.finish(val)
}

fn build_step(c: &FinSumSubConsts) -> Expr {
    let mut b = EnvDeclBuilder::new();
    let (k_id, k) = b.fresh_local(c.base.nat.clone());
    ...
    let ih_lhs = c.sum(
        k.clone(),
        pointwise_sub(c, &b, k.clone(), ih_f.clone(), ih_g.clone()),
    );
    let ih_rhs = c.sub_rat(c.sum(k.clone(), ih_f), c.sum(k.clone(), ih_g));
    ...
    let fg_cast = pointwise_sub(c, &b, k.clone(), f_cast.clone(), g_cast.clone());
    ...
    let last_diff = c.sub_rat(f_last.clone(), g_last.clone());

    let lhs = Expr::app(
        Expr::app(c.base.rat_add.clone(), sum_fg.clone()),
        last_diff.clone(),
    );
    let mid = Expr::app(
        Expr::app(
            c.base.rat_add.clone(),
            c.sub_rat(sum_f.clone(), sum_g.clone()),
        ),
        last_diff,
    );
    let rhs = c.sub_rat(
        Expr::app(
            Expr::app(c.base.rat_add.clone(), sum_f.clone()),
            f_last.clone(),
        ),
        Expr::app(
            Expr::app(c.base.rat_add.clone(), sum_g.clone()),
            g_last.clone(),
        ),
    );

    let ih_app = Expr::app(Expr::app(ih, f_cast), g_cast);
    let step1_fn = add_right_fn(c, &b, c.sub_rat(f_last.clone(), g_last.clone()));
    let step1 = congr_rat(
        c,
        c.base.rat.clone(),
        sum_fg,
    ...
    let step2 = Expr::apps(c.rat_sub_add_sub.clone(), [sum_f, sum_g, f_last, g_last]);
    let proof = eq_trans(c, lhs, mid, rhs, step1, step2);
...
fn build_value(c: &FinSumSubConsts) -> Expr {
...
impl Environment {
    /// Register `Fin.sum_sub` as a kernel-checked theorem.
    pub(crate) fn register_fin_sum_sub_theorem(&mut self) -> Result<(), EnvError> {
        let name = Name::from_string("Fin.sum_sub");
