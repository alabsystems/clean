...
//!
//! Registers the kernel-level axiom surfaces (S41-S43, S46, S50) as spec
//! definitions so that the `ProofLibrary` dependency audit can track them.
//!
//! These theorems are currently kernel axioms without constructive proofs.
//! They are registered as `AxiomCategory::FoundationalRule` with
//! `ProofStatus::Axiom` to reflect their actual trust status.
//!
...
impl Specification {
    ...
    ///
    /// These are kernel-level axioms from `Environment::init_boolean_analysis()`.
    ...
    pub(crate) fn add_boolean_analysis_spec(&mut self) -> Result<(), SpecError> {
        ...
        self.register_existing_definition(SpecDefinition {
            ...
            type_src: "(kernel axiom — type from environment)".to_string(),
            value_src: None,
            is_axiom: true,
            ...
            description: "S41: Parseval's identity — the sum of squared Fourier \
                          coefficients equals E[f^2]. Kernel axiom. \
        ...
        self.register_existing_definition(SpecDefinition {
            ...
            description: "S42: Influence-Fourier identity — the influence of variable i \
                          equals sum of hat{f}(S)^2 over S containing i. Kernel axiom. \
        ...
        self.register_existing_definition(SpecDefinition {
            ...
            description: "S46: Total influence identity — I(f) = sum_S |S| * hat{f}(S)^2. \
                          Kernel axiom. Ref: O'Donnell, Proposition 2.18."
                .to_string(),
            elaborated_type: None,
            elaborated_value: None,
            dependencies: None,
        ...
        self.register_existing_definition(SpecDefinition {
            ...
            description: "S50: Bonami-Beckner hypercontractivity — ||T_rho f||_q <= ||f||_p \
                          when rho^2 <= (p-1)/(q-1). Kernel axiom. \
