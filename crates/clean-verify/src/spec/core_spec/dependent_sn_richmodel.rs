    ...
    pub(super) fn add_dependent_sn_richmodel(&mut self) -> Result<(), SpecError> {
        ...
        self.add_inductive(
            concat!(
                ...
                "| pi : forall (G : ListType KExpr) (A : KExpr) (B : KExpr) (n : Level) (m : Level), KernelInfers tenv G A (KExpr.sort n) -> KernelInfers tenv (ListType.cons KExpr A G) B (KExpr.sort m) -> KernelInfers tenv G (KExpr.pi A B) (KExpr.sort (Level.imax n m))\n",
                "| lam : forall (G : ListType KExpr) (A : KExpr) (b : KExpr) (B : KExpr) (u : Level), KernelInfers tenv G A (KExpr.sort u) -> KernelInfers tenv (ListType.cons KExpr A G) b B -> KernelInfers tenv G (KExpr.lam A b) (KExpr.pi A B)\n",
