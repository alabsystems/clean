...
impl BinOp {
    ...
    /// Check if this is a (non-assignment) bit-shift operator.
    ///
    /// Per C11 6.5.7, shift operators do not undergo the usual arithmetic
    /// conversions: the operands are integer-promoted independently and the
...
/// Bit-field selector attached to an [`LValue`] that designates a sub-range of
/// bits within the storage unit at `LValue::ptr`.
///
/// Bits are counted from the least-significant bit of the storage unit (the
/// conventional GCC layout on little-endian targets). `ty` (on the parent
/// `LValue`) is the bit-field's declared storage-unit type and drives the read
/// width and signedness of the extracted value.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct BitFieldRef {
    ...
    pub bit_width: usize,
    /// Size (in bytes) of the storage unit.
...
pub struct LValue {
    /// Pointer to the location (storage unit for bit-fields)
    pub ptr: Pointer,
    /// Type of the value at this location (storage-unit type for bit-fields)
    pub ty: CType,
    /// Bit-field selector, when this lvalue names a bit-field member.
    pub bitfield: Option<BitFieldRef>,
}

impl LValue {
    /// Create a new (non bit-field) LValue
    ...
    /// Create an LValue naming a bit-field within the storage unit at `ptr`.
    pub fn bitfield(ptr: Pointer, ty: CType, bitfield: BitFieldRef) -> Self {
        Self {
            ptr,
            ty,
...
impl ExprResult {
    ...
    /// Create an lvalue result naming a bit-field.
    pub fn lvalue_bitfield(ptr: Pointer, ty: CType, bitfield: BitFieldRef) -> Self {
        ExprResult::LValue(LValue::bitfield(ptr, ty, bitfield))
    }
...
mod tests {
    ...
    fn test_binop_properties() {
        ...
        // Compound shift-assignments are assignments, not plain shifts.
        assert!(!BinOp::ShlAssign.is_shift());
        assert!(!BinOp::ShrAssign.is_shift());
    }
