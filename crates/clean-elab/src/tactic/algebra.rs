...
/// ENSURES: Returns `(lhs, rhs)` only for equality expressions accepted by
/// `match_equality`.
/// ENSURES: Equality type and universe levels are discarded from the result.
...
/// ENSURES: Returns `true` iff the top-level constructor of `expr` is
/// `ExprKind::Pi`.
...
impl Default for AbelConfig {
    /// ENSURES: Default config uses a 100-step bound and disables verbose
    /// logging.
    fn default() -> Self {
        Self {
            max_steps: 100,
...
impl AbelTerm {
    /// ENSURES: Returns an empty normalized term with no coefficients or
    /// variables.
    ...
    /// ENSURES: Returns the additive identity term.
    pub(crate) fn zero() -> Self {
        Self::new()
    }

    /// REQUIRES: `var_idx` is the caller's canonical slot for `expr`.
    /// ENSURES: Returns a term containing exactly one variable with coefficient
    /// `1`.
    ...
    /// REQUIRES: `self` and `other` use compatible variable indices.
    /// ENSURES: Matching coefficients are summed and zero coefficients are
    /// removed.
    /// ENSURES: Result variables are the union of both variable lists modulo
    /// `exprs_equal`.
    ...
    /// REQUIRES: `self` and `other` use compatible variable indices.
    /// ENSURES: Equivalent to `self.add(&other.negate())`.
    pub(crate) fn sub(&self, other: &Self) -> Self {
        self.add(&other.negate())
    }

    /// ENSURES: All coefficients in the returned term have the opposite sign.
    ...
    /// ENSURES: Returns `true` iff the term has no nonzero coefficients.
...
/// ENSURES: Equivalent to `abel_with_config(state, AbelConfig::default())`.
pub fn abel(state: &mut ProofState) -> TacticResult {
    abel_with_config(state, AbelConfig::default())
}

/// REQUIRES: `state.goals` is non-empty.
/// REQUIRES: The current goal target is an equality if `Ok(())` is expected.
/// ENSURES: On `Ok(())`, both sides normalize to the same abelian form and the
/// goal is closed with `rfl`.
/// ENSURES: On `Err(GoalMismatch)`, the goal target is not an equality.
/// ENSURES: On `Err(ArithmeticFailed)`, normalization found a nonzero
/// difference.
pub fn abel_with_config(state: &mut ProofState, config: AbelConfig) -> TacticResult {
    ...
    let mut var_map: std::collections::HashMap<Expr, usize> = std::collections::HashMap::new();
    ...
    if !diff.is_zero() {
        return Err(TacticError::ArithmeticFailed {
            tactic: "abel".to_string(),
            reason: format!(
                "terms do not normalize to equal values. Difference: {:?}",
                diff.coefficients
            ),
        });
    }
...
/// REQUIRES: `var_counter`, `var_map`, and `vars` describe a shared
/// variable-indexing table for one normalization pass.
/// ENSURES: Recognized add/sub/neg/zero syntax is normalized into an
/// `AbelTerm`.
/// ENSURES: Unrecognized subexpressions are interned as atomic variables and
/// inserted into `var_map`/`vars` at most once.
/// ENSURES: Recursive descent runs under `stack_safe`.
fn parse_abel_term(
    expr: &Expr,
    var_counter: &mut usize,
    var_map: &mut std::collections::HashMap<Expr, usize>,
    vars: &mut Vec<Expr>,
    _config: &AbelConfig,
    ...
    stack_safe(|| match expr.kind() {
        ExprKind::App(f, b) => {
            if let ExprKind::App(f2, a) = f.kind() {
                ...
                if let ExprKind::App(f3, _ty) = f.kind() {
                    if let ExprKind::Const(name, _) = f3.kind() {
            ...
            if let ExprKind::App(f2, _ty) = f.kind() {
            ...
            let idx = *var_map.entry(expr.clone()).or_insert_with(|| {
                let idx = *var_counter;
                *var_counter += 1;
        ...
        ExprKind::Const(name, _)
            if name.to_string().contains("zero") || name.to_string() == "0" =>
        {
            Ok(AbelTerm::zero())
        }
        ExprKind::Lit(clean_kernel::Literal::Nat(n)) if n.to_u64() == Some(0) => {
            Ok(AbelTerm::zero())
        }
        _ => {
            let idx = *var_map.entry(expr.clone()).or_insert_with(|| {
...
impl GroupTerm {
    /// ENSURES: Returns the multiplicative identity term.
    ...
    /// REQUIRES: `var_idx` is the caller's canonical slot for `expr`.
    /// ENSURES: Returns a term containing exactly one factor with exponent
    /// `1`.
    pub(crate) fn single(var_idx: usize, expr: Expr) -> Self {
        Self {
    ...
    /// REQUIRES: `self` and `other` use compatible variable indices.
    /// ENSURES: Matching factors have exponents added, zero exponents are
    /// removed, and the result is index-sorted.
    /// ENSURES: Result variables are the union of both variable lists modulo
    /// `exprs_equal`.
    ...
    /// ENSURES: All exponents in the returned term have the opposite sign and
    /// factor order is reversed.
    ...
    /// ENSURES: Returns the identity term when `n == 0`.
    /// ENSURES: Otherwise multiplies every exponent by `n` and drops any zero
    /// exponents.
    ...
    /// ENSURES: Returns `true` iff the term has no nonzero exponents.
    pub(crate) fn is_identity(&self) -> bool {
        self.factors.is_empty() || self.factors.iter().all(|&(_, e)| e == 0)
    }

    /// REQUIRES: `self` and `other` use compatible variable indices.
    /// ENSURES: Returns `true` iff both terms have the same factor multiset
    /// after sorting by variable index.
...
/// REQUIRES: `state.goals` is non-empty.
/// ENSURES: Equivalent to `group_with_config(state, GroupConfig::default())`.
...
/// REQUIRES: `state.goals` is non-empty.
/// REQUIRES: The current goal target is an equality if `Ok(())` is expected.
/// ENSURES: On `Ok(())`, both sides normalize compatibly and the goal is
/// closed with `rfl`.
/// ENSURES: On `Err(GoalMismatch)`, the goal target is not an equality.
/// ENSURES: On `Err(ArithmeticFailed)`, normalization found incompatible
/// factor/exponent multisets.
pub fn group_with_config(state: &mut ProofState, config: GroupConfig) -> TacticResult {
    ...
    if !diff.is_identity() && !lhs_term.eq_abelian(&rhs_term) {
        return Err(TacticError::ArithmeticFailed {
            tactic: "group".to_string(),
            reason: format!(
                "terms do not normalize to equal values. lhs: {:?}, rhs: {:?}",
...
/// REQUIRES: `var_counter`, `var_map`, and `vars` describe a shared
/// variable-indexing table for one normalization pass.
/// ENSURES: Recognized mul/div/inv/pow/one syntax is normalized into a
/// `GroupTerm`.
/// ENSURES: Unrecognized subexpressions are interned as atomic variables and
/// inserted into `var_map`/`vars` at most once.
/// ENSURES: Recursive descent runs under `stack_safe`.
fn parse_group_term(
    ...
    stack_safe(|| match expr.kind() {
        ...
        ExprKind::Const(name, _) if name.to_string().contains("one") || name.to_string() == "1" => {
            Ok(GroupTerm::identity())
        }
        ExprKind::Lit(clean_kernel::Literal::Nat(n)) if n.to_u64() == Some(1) => {
            Ok(GroupTerm::identity())
        }
        _ => {
            let idx = *var_map.entry(expr.clone()).or_insert_with(|| {
