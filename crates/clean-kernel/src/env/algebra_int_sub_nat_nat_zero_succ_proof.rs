...
//!    ∀ n : Nat, Eq Int (Int.subNatNat Nat.zero (Nat.succ n)) (Int.negSucc n)`.
//!
//! This is the first variable-argument negative-branch normalization
//! theorem for `Int.subNatNat`. Unlike the zero-right and mixed-sign
//! branch equations, it requires induction over `n`.
...
struct IntSubNatNatZeroSuccConsts {
    int_type: Expr,
    nat_type: Expr,
    nat_zero: Expr,
    nat_succ: Expr,
...
impl IntSubNatNatZeroSuccConsts {
    fn new() -> Self {
    ...
    fn sub_zero_succ(&self, n: Expr) -> Expr {
        Expr::app(
            Expr::app(self.int_sub_nat_nat.clone(), self.nat_zero.clone()),
...
fn build_decrement_fn(c: &IntSubNatNatZeroSuccConsts) -> Expr {
    ...
    let neg_succ_case = {
        let mut b = EnvDeclBuilder::new();
        let (p_id, p) = b.fresh_local(c.nat_type.clone());
        let body = c.neg_succ(Expr::app(c.nat_succ.clone(), p));
        let lam = b.mk_lam(p_id, BinderInfo::Default, c.nat_type.clone(), body);
        b.finish(lam)
    };
...
fn build_int_sub_nat_nat_zero_succ_type(c: &IntSubNatNatZeroSuccConsts) -> Expr {
    let mut b = EnvDeclBuilder::new();
    let (n_id, n) = b.fresh_local(c.nat_type.clone());
    let concl = c.eq_int(c.sub_zero_succ(n.clone()), c.neg_succ(n));
    let ty_raw = b.mk_pi(n_id, BinderInfo::Default, c.nat_type.clone(), concl);
    b.finish(ty_raw)
}

fn build_int_sub_nat_nat_zero_succ_value(c: &IntSubNatNatZeroSuccConsts) -> Expr {
    ...
    let motive = {
        ...
        let body = c.eq_int(c.sub_zero_succ(t.clone()), c.neg_succ(t));
    ...
    let base = Expr::apps(
        c.eq_refl.clone(),
        [c.int_type.clone(), c.neg_succ(c.nat_zero.clone())],
    );

    let step = {
        let mut sb = EnvDeclBuilder::child_of(&b);
        let (k_id, k) = sb.fresh_local(c.nat_type.clone());
        let ih_type = c.eq_int(c.sub_zero_succ(k.clone()), c.neg_succ(k.clone()));
...
impl Environment {
    /// Register `Int.subNatNat_zero_succ` as a kernel-checked theorem.
    ///
    /// The proof inducts on `n`. The base case closes by `Eq.refl`; the
    /// step maps the induction hypothesis through the one-step
    /// `Int.subNatNat` decrement function with `congrArg`.
    pub(crate) fn register_int_sub_nat_nat_zero_succ_proof(&mut self) -> Result<(), EnvError> {
        ...
        let name = Name::from_string("Int.subNatNat_zero_succ");
        ...
        let c = IntSubNatNatZeroSuccConsts::new();
        let type_ = build_int_sub_nat_nat_zero_succ_type(&c);
        let value = build_int_sub_nat_nat_zero_succ_value(&c);
...
mod tests {
    ...
    #[test]
    fn test_int_sub_nat_nat_zero_succ_registered_as_theorem() {
        let mut env = Environment::new();
        env.register_int_sub_nat_nat_zero_succ_proof()
            .expect("first registration");
        env.register_int_sub_nat_nat_zero_succ_proof()
            .expect("idempotent re-registration");

        let info = env
            .get_const(&Name::from_string("Int.subNatNat_zero_succ"))
            .expect("Int.subNatNat_zero_succ should be registered");
        assert_eq!(info.kind, ConstantKind::Theorem);
        assert!(info.value.is_some(), "Theorem must retain its proof value");
    }

    #[test]
    fn test_int_sub_nat_nat_zero_succ_axiom_closure_is_empty() {
        let mut env = Environment::new();
        env.init_int_arith_lemmas()
            .expect("Int arithmetic lemmas should initialize");

        let quality = env
            .proof_quality(&Name::from_string("Int.subNatNat_zero_succ"))
            .expect("proof_quality should be reported");
        assert!(
            matches!(quality, ProofQuality::Constructive),
            "Int.subNatNat_zero_succ must be Constructive, got {:?}",
            quality
        );
    }
}
