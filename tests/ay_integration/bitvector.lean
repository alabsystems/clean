/-
  Ay Integration Tests: Bitvector Arithmetic (QF_BV)

  Tests the ay_bv tactic for fixed-width bitvector goals.
-/
...
theorem perf_chain_bv (x y z : UInt16) :
    ((x &&& y) ||| z) ^^^ ((x ||| y) &&& z) =
    ((x ^^^ y) &&& (y ^^^ z)) ||| ((x ^^^ z) &&& (y ^^^ x)) := by
  ay_bv  -- This may take longer; marks performance boundary
