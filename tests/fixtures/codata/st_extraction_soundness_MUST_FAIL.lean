-- NEGATIVE CONTROL for st_extraction_soundness.lean. MUST FAIL TO CHECK.
--
-- Identical except the emitted step DOUBLES where the source increments. If
-- the observational theorem still went through, it would be insensitive to
-- the term it names.
--
-- Rank-7 B7 negative control, plain lane.
--
...
def countIR : IRCorec :=
  { observe := IROp.slot0,
    step0 := IROp.add IROp.slot0 IROp.slot0,
    step1 := IROp.lit 0 }
