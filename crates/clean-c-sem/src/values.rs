...
impl CValue {
    ...
    pub fn zero(ty: &CType) -> Self {
        match ty {
            ...
            // A flexible array member (`T[]`) has no elements of its own, so
            // its zero value is an empty array (it contributes no storage to
            // the enclosing struct; C99 6.7.2.1p18).
            CType::IncompleteArray(_) => CValue::Array(Vec::new()),
    ...
    /// Check if this is an arithmetic value (integer, boolean, or floating).
    ///
    /// Arithmetic values are subject to the usual arithmetic conversions; this
    /// excludes pointers, aggregates, and `Undef`.
...
impl CValue {
    ...
    pub fn ne(&self, other: &CValue) -> UBResult<CValue> {
        ...
        match eq_result {
            ...
            _ => unreachable!("eq() returned non-boolean CValue"),
        }
    }
...
mod tests {
    ...
    fn test_to_bool() {
        ...
        assert!(
            matches!(CValue::Undef.to_bool(), Err(UBKind::UninitializedRead)),
            "Undef.to_bool() should produce UninitializedRead"
        );
    }
