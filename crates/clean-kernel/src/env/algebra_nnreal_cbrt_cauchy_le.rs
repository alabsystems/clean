...
//! KKL real/cbrt layer (ported from the sqrt layer; cbrtDyadicApprox mirrors dyadicApprox) — the `p ≤ q` two-sided dyadic bounds
...
//! - `Rat.cbrtDyadicApprox_le_add : ∀ x n d,
//!       Rat.le (Rat.cbrtDyadicApprox x n) (Rat.cbrtDyadicApprox x (Nat.add n d))`.
//!   (monotone over the gap)
//! - `Rat.cbrtDyadicApprox_mono : ∀ x p q, Nat.le p q →
//!       Rat.le (Rat.cbrtDyadicApprox x p) (Rat.cbrtDyadicApprox x q)`.
//! - `Rat.cbrtDyadicApprox_le_add_inv_of_le : ∀ x p q, Nat.le p q →
//!       Rat.le (Rat.cbrtDyadicApprox x q)
//!              (Rat.add (Rat.cbrtDyadicApprox x p) (inv (ofNat 2^p)))`.
//!
...
impl CbrtLeConsts {
    pub(crate) fn new() -> Self {
        ...
        Self {
            ...
            rat_dyadic_approx_le_succ: k("Rat.cbrtDyadicApprox_le_succ"),
            rat_dyadic_approx_le_add: k("Rat.cbrtDyadicApprox_le_add"),
            rat_dyadic_approx_le_add_inv: k("Rat.cbrtDyadicApprox_le_add_inv"),
...
impl Environment {
    ...
    pub fn init_algebra_nnreal_cbrt_cauchy_le(&mut self) -> Result<(), EnvError> {
        self.init_eq()?;
        self.init_exists()?;
        self.init_nat()?;
        self.init_le()?; // Nat.le, Nat.le.refl, Nat.le.step, Nat.le.rec
        self.init_algebra_nnreal_cbrt_seq()?; // dyadicApprox, ofNat, Nat.pow
        self.init_algebra_nnreal_cbrt_cauchy_mono()?; // dyadicApprox_le_succ
        self.init_algebra_nnreal_cbrt_cauchy_tele()?; // dyadicApprox_le_add_inv
        self.init_rat_quotient_poc()?; // Rat.le_refl, Rat.le_trans

        let c = CbrtLeConsts::new();
        self.register_cbrt_nat_exists_eq_add_of_le(&c)?;
        self.register_cbrt_dyadic_approx_le_add(&c)?;
        self.register_cbrt_dyadic_approx_mono(&c)?;
        self.register_cbrt_dyadic_approx_le_add_inv_of_le(&c)?;
        Ok(())
    }

    /// `Nat.exists_eq_add_of_le : ∀ p q, Nat.le p q → ∃ d, Eq Nat q (Nat.add p d)`.
    fn register_cbrt_nat_exists_eq_add_of_le(&mut self, c: &CbrtLeConsts) -> Result<(), EnvError> {
    ...
    /// `Rat.cbrtDyadicApprox_le_add : ∀ x n d, le (a_n)(a_(n+d))`.
    /// `Nat.rec` on `d` (n fixed), last-step peel via `dyadicApprox_le_succ`.
    fn register_cbrt_dyadic_approx_le_add(&mut self, c: &CbrtLeConsts) -> Result<(), EnvError> {
        let name = Name::from_string("Rat.cbrtDyadicApprox_le_add");
        if self.get_const(&name).is_some() {
            return Ok(());
        }
        let ty = {
    ...
    /// `Rat.cbrtDyadicApprox_mono : ∀ x p q, Nat.le p q → le (a_p)(a_q)`.
    fn register_cbrt_dyadic_approx_mono(&mut self, c: &CbrtLeConsts) -> Result<(), EnvError> {
        let name = Name::from_string("Rat.cbrtDyadicApprox_mono");
        if self.get_const(&name).is_some() {
            return Ok(());
        }
        let ty = {
    ...
    /// `Rat.cbrtDyadicApprox_le_add_inv_of_le : ∀ x p q, Nat.le p q →
    ///     le (a_q) (a_p + inv(2^p))`.
    fn register_cbrt_dyadic_approx_le_add_inv_of_le(
        &mut self,
        c: &CbrtLeConsts,
    ) -> Result<(), EnvError> {
        let name = Name::from_string("Rat.cbrtDyadicApprox_le_add_inv_of_le");
        if self.get_const(&name).is_some() {
            return Ok(());
        }
        let ty = {
...
mod tests {
    ...
    fn env() -> Environment {
        ...
        env.init_algebra_nnreal_cbrt_cauchy_le()
            .expect("init_algebra_nnreal_cbrt_cauchy_le");
        env.init_algebra_nnreal_cbrt_cauchy_le()
            .expect("idempotent");
        env
    }
