...
const SRC_IR_H2_B2: &str = "def ir_h2_b2 : IRBlock := IRBlock.mk ir_d2 ir_nl0 (ir_bd2 (ir_nd1 (IRInst.const_ ir_tBool (IRConst.bool_ Bool.true)) ir_d5) (ir_nd (IRInst.br ir_d4 (ir_nl1 ir_d5))))";

const SRC_IR_H2_B3: &str = "def ir_h2_b3 : IRBlock := IRBlock.mk ir_d3 ir_nl0 (ir_bd2 (ir_nd1 (IRInst.const_ ir_tBool (IRConst.bool_ Bool.false)) ir_d6) (ir_nd (IRInst.br ir_d4 (ir_nl1 ir_d6))))";

const SRC_IR_H2_B4: &str = "def ir_h2_b4 : IRBlock := IRBlock.mk ir_d4 (ir_nl1 ir_d1) (ir_bd1 (ir_nd (IRInst.ret (ir_nl1 ir_d1))))";

const SRC_IR_H2_FUNC: &str = "def ir_h2_func : IRFunc := IRFunc.mk ir_d0 (ir_nl1 ir_d0) ir_d0 (ir_blk ir_h2_b0 (ir_blk ir_h2_b1 (ir_blk ir_h2_b2 (ir_blk ir_h2_b3 (ir_blk ir_h2_b4 ir_blk0)))))";

const SRC_IR_H2_MODULE: &str = "def ir_h2_module : IRModule := IRModule.mk (IRList.cons IRFunc ir_h2_func (IRList.nil IRFunc)) (IRList.nil IRGlobal)";
...
impl Specification {
    ...
    pub(super) fn add_eval_ir_mode(&mut self) -> Result<(), SpecError> {
        ...
        self.add_recursive_def(SRC_IR_H2_B3, "ir_h2_b3: the default edge => false. Note there is NO unreachable block in the emitted body: exhaustiveness is not expressed as a trap, it is expressed by the default edge carrying the false answer. DerivedProved, zero axiom_deps.")?;
        self.add_recursive_def(SRC_IR_H2_B4, "ir_h2_b4: the JOIN block, taking a block PARAMETER. The emitted body funnels all three arms through br into bb4(%1: bool) and returns that parameter, rather than returning from each arm. The Level module never used block parameters; this one must, because the shipped body does. DerivedProved, zero axiom_deps.")?;
        self.add_recursive_def(SRC_IR_H2_FUNC, "ir_h2_func: CleanMode::has_cubical_layer as EvalIR, five blocks, matching the emitted body's control-flow graph exactly. DerivedProved, zero axiom_deps.")?;
        self.add_recursive_def(SRC_IR_H2_MODULE, "ir_h2_module: the module for has_cubical_layer, TRANSCRIBED FROM MEASURED OUTPUT (docs/analysis/a0-has-cubical-layer-measured-2026-08-11.md) rather than from a reading of the Rust source. The previous hand-authored version was semantically equivalent but structurally different -- six switch cases, one shared true block, direct returns, an unreachable default -- and a theorem about it was NOT a theorem about the shipped body. DerivedProved, zero axiom_deps.")?;
...
mod tests {
    ...
    fn test_switch_matches_emitted_two_cases_plus_default() {
        assert!(SRC_IR_H2_B0.contains("(ir_sc ir_d2 ir_d1 (ir_sc ir_d3 ir_d2 ir_sc0))"));
        assert!(
            !SRC_IR_H2_B0.contains("ir_sc ir_d4"),
    ...
    fn test_join_block_takes_a_parameter() {
        assert!(SRC_IR_H2_B4.contains("IRBlock.mk ir_d4 (ir_nl1 ir_d1)"));
        assert!(SRC_IR_H2_B1.contains("IRInst.br ir_d4"));
        assert!(SRC_IR_H2_B2.contains("IRInst.br ir_d4"));
        assert!(SRC_IR_H2_B3.contains("IRInst.br ir_d4"));
    }
    ...
    fn test_two_distinct_true_arms() {
        assert!(SRC_IR_H2_B1.contains("IRConst.bool_ Bool.true"));
        assert!(SRC_IR_H2_B2.contains("IRConst.bool_ Bool.true"));
        assert_ne!(SRC_IR_H2_B1, SRC_IR_H2_B2);
    }
