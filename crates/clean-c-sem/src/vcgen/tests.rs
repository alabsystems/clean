...
fn test_vc_to_clean() {
    ...
    let lean_expr = vc_to_clean(&vc);

    // Should produce a clean expression (App for GE.ge)
    assert!(matches!(
        lean_expr.kind(),
...
fn test_block_wp() {
    ...
    let postcond = Spec::ge(Spec::var("y"), Spec::int(0));
    let wp = vcgen.wp_stmt(&block, &postcond, None);

    let expected = Spec::ge(
        Spec::binop(BinOp::Add, Spec::int(1), Spec::int(1)),
        Spec::int(0),
    );
    assert_eq!(
        wp, expected,
        "block WP should substitute assignments from right to left"
    );
}
...
fn test_accumulator_pattern_mul_assign() {
    ...
    let inv = inference
        .detect_sum_pattern(&body, &context)
        .expect("Should detect product accumulator pattern");
    // Mul returns conjunction: product >= 0 AND (init >= 0 → product >= 0)
    assert!(
        matches!(inv, Spec::And(_)),
        "Product invariant should be conjunction, got {inv:?}"
    );
}
...
fn test_accumulator_pattern_sum_form() {
    ...
    let inv = inference
        .detect_sum_pattern(&body, &context)
        .expect("Should detect sum = sum + x pattern");
    // Without explicit initial, defaults to sum >= 0
    assert!(
        matches!(inv, Spec::BinOp { op: BinOp::Ge, .. }),
        "Sum form invariant should be >= comparison, got {inv:?}"
    );
}
...
fn test_bitwise_accumulator() {
    ...
    let inv = inference
        .detect_sum_pattern(&body, &context)
        .expect("Should detect bitwise or accumulator pattern");
    // BitwiseOr preserves boundedness: flags >= 0
    assert!(
        matches!(inv, Spec::BinOp { op: BinOp::Ge, .. }),
        "Bitwise OR invariant should be >= comparison, got {inv:?}"
    );
}
...
fn test_min_pattern_in_block() {
    ...
    let inv = inference
        .detect_sum_pattern(&body, &context_with_init)
        .expect("Should detect min pattern with initial value");
    // Min with initial=100: min <= 100
    assert!(
        matches!(inv, Spec::BinOp { op: BinOp::Le, .. }),
        "Min invariant should be <= comparison, got {inv:?}"
    );
...
fn test_extract_array_accesses() {
    ...
    let (arr_name_w, _, _) = accesses
        .iter()
        .find(|(_, idx, is_write)| matches!(idx, CExpr::Var(v) if v == "i") && *is_write)
        .expect("Should find write to arr[i]");
    assert_eq!(arr_name_w, "arr", "Write access should be on array 'arr'");

    // arr[j] is read (is_write = false)
    let (arr_name_r, _, _) = accesses
        .iter()
        .find(|(_, idx, is_write)| matches!(idx, CExpr::Var(v) if v == "j") && !*is_write)
        .expect("Should find read from arr[j]");
    assert_eq!(arr_name_r, "arr", "Read access should be on array 'arr'");
}
...
fn test_spec_to_cexpr_non_convertible() {
    ...
    assert!(
        vcgen.spec_to_cexpr(&Spec::True).is_none(),
        "Spec::True should not convert to CExpr"
    );
    assert!(
        vcgen.spec_to_cexpr(&Spec::False).is_none(),
        "Spec::False should not convert to CExpr"
    );
    assert!(
        vcgen.spec_to_cexpr(&Spec::Result).is_none(),
        "Spec::Result should not convert to CExpr"
    );
    assert!(
        vcgen.spec_to_cexpr(&Spec::old(Spec::var("x"))).is_none(),
        "Spec::old should not convert to CExpr"
    );
    assert!(
        vcgen
            .spec_to_cexpr(&Spec::forall("x", CType::int(), Spec::True))
            .is_none(),
        "Spec::forall should not convert to CExpr"
    );
    assert!(
        vcgen
            .spec_to_cexpr(&Spec::exists("x", CType::int(), Spec::True))
            .is_none(),
        "Spec::exists should not convert to CExpr"
    );
    assert!(
        vcgen
            .spec_to_cexpr(&Spec::implies(Spec::True, Spec::True))
            .is_none(),
        "Spec::implies should not convert to CExpr"
    );
    assert!(
        vcgen.spec_to_cexpr(&Spec::and(vec![Spec::True])).is_none(),
        "Spec::and should not convert to CExpr"
    );
    assert!(
        vcgen.spec_to_cexpr(&Spec::or(vec![Spec::True])).is_none(),
        "Spec::or should not convert to CExpr"
    );
    assert!(
        vcgen.spec_to_cexpr(&Spec::not(Spec::True)).is_none(),
        "Spec::not should not convert to CExpr"
    );
    assert!(
        vcgen.spec_to_cexpr(&Spec::valid(Spec::var("p"))).is_none(),
        "Spec::valid should not convert to CExpr"
    );
}
