...
use clean_olean::{parse_header, parse_module, ConstantKind};
use std::fs;
...
#[test]
fn test_module_parsing_propagates_errors() {
    // Verify that parse_module returns Err for truncated/corrupt data
    // rather than silently returning partial results
    let bytes = read_fixture("custom/Minimal.olean");

    // Truncate just before the end of compacted region
    // This should cause an error during array parsing, not silent partial success
    let truncated_mid = &bytes[..bytes.len() / 2];
    let result = parse_module(truncated_mid);

    // Should fail with error, not succeed with partial data
    // If this returns Ok with fewer constants than expected, error propagation is broken
    assert!(
        result.is_err(),
        "Truncated module should return Err, not partial Ok"
    );
}

#[test]
fn test_module_parsing_complete_not_partial() {
    // Verify that successful parsing returns all expected data
    let bytes = read_fixture("custom/Minimal.olean");
    let module = parse_module(&bytes).expect("Valid module should parse");
    let const_names: Vec<&str> = module.constants.iter().map(|c| c.name.as_str()).collect();

    // Minimal.olean contains two user declarations plus generated cstage helpers.
    assert_eq!(
        module.constants.len(),
        5,
        "Expected exactly 5 constants for Minimal.olean fixture, got {}: {const_names:?}",
        module.constants.len()
    );

    // Verify imports were also parsed (not silently skipped)
    // Minimal.olean imports Init
    assert!(
        !module.imports.is_empty(),
        "Expected imports, got none - possible silent failure"
    );
}
