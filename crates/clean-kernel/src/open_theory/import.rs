...
fn type_schema_key(ty: &OtType) -> String {
    let mut vars = Vec::new();
    let mut out = String::new();
    write_type_schema(ty, &mut vars, &mut out);
    out
}

fn write_type_schema(ty: &OtType, vars: &mut Vec<OtName>, out: &mut String) {
    match ty {
        OtType::Var(name) => {
            let index = vars
                .iter()
        ...
        OtType::Bool => out.push_str("bool"),
        OtType::Function { domain, codomain } => {
            out.push('(');
            write_type_schema(domain, vars, out);
            out.push_str("->");
            write_type_schema(codomain, vars, out);
            out.push(')');
        }
        OtType::App { op, args } => {
            out.push_str(&op.name.as_dotted());
            out.push('[');
            for (index, arg) in args.iter().enumerate() {
                if index > 0 {
                    out.push(',');
                }
                write_type_schema(arg, vars, out);
            }
            out.push(']');
        }
    }
