...
use crate::commit::encoding::encode_cert;
use crate::commit::error::{CommitError, VerifyError};
use crate::commit::ProofCommitmentScheme;
...
impl IpaScheme {
    ...
    pub fn setup(max_degree: usize) -> Result<Self, CommitError> {
        ...
        let generators = (0..size)
            .map(|i| hash_to_g1(&format!("clean-ipa-generator-{i}")))
            .collect();

        let h = hash_to_g1("clean-ipa-blinding");
        let u = hash_to_g1("clean-ipa-inner-product");
    ...
    pub fn commit_poly(&self, poly: &DensePolynomial<Fr>) -> Result<IpaCommitment, CommitError> {
        ...
        let commitment: G1Projective = coeffs
            ...
            .fold(G1Projective::zero(), |acc, (coeff, generator)| {
                acc + G1Projective::from(*generator) * coeff
            });
...
mod tests {
    ...
    fn test_ipa_with_cert() {
        ...
        // Verify the commitment-open-verify roundtrip
        let verified = ipa.verify(&commitment, point, value, &proof).unwrap();
        assert!(verified, "IPA verify should accept a valid open proof");
    }
    ...
    fn test_ipa_batch_verify_valid() {
        ...
        assert!(single1, "single verify for poly1 should succeed");
        assert!(single2, "single verify for poly2 should succeed");

        // Batch verify should also succeed
        let batch_result = ipa
            .batch_verify(&batch)
            .expect("batch verify should not error");
        assert!(batch_result, "batch verify should accept valid proofs");
    }
