...
/// Find a cycle in the graph using iterative DFS with color marking.
///
/// Returns `PropagationError::CycleDetected` with a node on the cycle.
fn find_cycle(edges: &[Vec<u32>]) -> PropagationError {
    let n = edges.len();
    // Color: 0 = white (unvisited), 1 = gray (in-progress), 2 = black (finished)
    let mut color = vec![0u8; n];

    // Iterative DFS using an explicit stack.
    // Each stack entry is (node, child_iterator_index).
    for start in 0..n {
        if color[start] != 0 {
            continue;
        }
        ...
        while let Some((node, child_idx)) = stack.last_mut() {
            if *child_idx < edges[*node].len() {
                let child = edges[*node][*child_idx] as usize;
                *child_idx += 1;

                match color[child] {
                    0 => {
                        // Unvisited: push and continue DFS
                        color[child] = 1;
                        stack.push((child, 0));
                    }
                    1 => {
                        // Back edge: cycle found
                        return PropagationError::CycleDetected { node: child as u32 };
                    }
                    _ => {
                        // Already finished: cross/forward edge, no cycle here
                    }
                }
            } else {
    ...
    // Should not reach here if called from propagate() on detected non-convergence,
    // but handle gracefully.
    PropagationError::CycleDetected { node: 0 }
}
