...
//!
//! This closes one positive outer / mixed inner-positive branch of the
...
//! `m + succ k` to `k + succ m` with checked Nat arithmetic.
...
struct IntAddAssocOfNatSuccNegSuccOfNatSuccConsts {
    int_type: Expr,
    nat_type: Expr,
    nat_succ: Expr,
    nat_add: Expr,
...
impl IntAddAssocOfNatSuccNegSuccOfNatSuccConsts {
    fn new() -> Self {
    ...
    fn lhs_mid(&self, m: Expr, n: Expr, k: Expr) -> Expr {
        self.add_int(self.sub_nat_nat(m, n), self.of_nat(self.succ(k)))
    }

    fn lhs_normal(&self, m: Expr, n: Expr, k: Expr) -> Expr {
        self.sub_nat_nat(self.nat_add(m, self.succ(k)), n)
    }

    fn rhs(&self, m: Expr, n: Expr, k: Expr) -> Expr {
        self.add_int(self.of_nat(self.succ(m)), self.inner_rhs(n, k))
    }

    fn rhs_mid(&self, m: Expr, n: Expr, k: Expr) -> Expr {
        self.add_int(self.of_nat(self.succ(m)), self.sub_nat_nat(k, n))
    }

    fn rhs_normal(&self, m: Expr, n: Expr, k: Expr) -> Expr {
        self.sub_nat_nat(self.nat_add(k, self.succ(m)), n)
    }
...
fn build_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_type(
    c: &IntAddAssocOfNatSuccNegSuccOfNatSuccConsts,
) -> Expr {
...
fn build_add_left_pos_fn(
    ...
    let mut fb = EnvDeclBuilder::child_of(parent);
    let (x_id, x) = fb.fresh_local(c.int_type.clone());
    let body = c.add_int(c.of_nat(c.succ(m)), x);
    let lam = fb.mk_lam(x_id, BinderInfo::Default, c.int_type.clone(), body);
    fb.finish_child(lam)
}
...
fn build_sub_left_fn(
    ...
    let body = c.sub_nat_nat(x, n);
    let lam = fb.mk_lam(x_id, BinderInfo::Default, c.nat_type.clone(), body);
    fb.finish_child(lam)
}

fn build_index_proof(c: &IntAddAssocOfNatSuccNegSuccOfNatSuccConsts, m: Expr, k: Expr) -> Expr {
    let add_m_succ_k = c.nat_add(m.clone(), c.succ(k.clone()));
    let succ_add_m_k = c.succ(c.nat_add(m.clone(), k.clone()));
    let succ_add_k_m = c.succ(c.nat_add(k.clone(), m.clone()));
    let add_k_succ_m = c.nat_add(k.clone(), c.succ(m.clone()));

    let h_add_succ_m_k = Expr::app(Expr::app(c.nat_add_succ.clone(), m.clone()), k.clone());
    let h_comm = Expr::app(Expr::app(c.nat_add_comm.clone(), m.clone()), k.clone());
    let h_comm_succ = Expr::apps(
        c.congr_arg.clone(),
        [
            c.nat_type.clone(),
            c.nat_type.clone(),
            c.nat_add(m.clone(), k.clone()),
            c.nat_add(k.clone(), m.clone()),
            c.nat_succ.clone(),
            h_comm,
        ],
    );
    let h_add_succ_k_m_forward = Expr::app(Expr::app(c.nat_add_succ.clone(), k.clone()), m.clone());
    let h_add_succ_k_m = Expr::apps(
        c.eq_symm.clone(),
        [
            c.nat_type.clone(),
            add_k_succ_m.clone(),
            succ_add_k_m.clone(),
            h_add_succ_k_m_forward,
        ],
    );
...
fn build_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_value(
    c: &IntAddAssocOfNatSuccNegSuccOfNatSuccConsts,
) -> Expr {
    let mut b = EnvDeclBuilder::new();
    let (m_id, m) = b.fresh_local(c.nat_type.clone());
    ...
    let h_index_nat = build_index_proof(c, m.clone(), k.clone());
    let sub_left_fn = build_sub_left_fn(c, &b, n.clone());
    let h_index = Expr::apps(
        c.congr_arg.clone(),
        [
            c.nat_type.clone(),
            c.int_type.clone(),
            c.nat_add(m.clone(), c.succ(k.clone())),
            c.nat_add(k.clone(), c.succ(m.clone())),
            sub_left_fn,
            h_index_nat,
        ],
    );
    ...
    let add_left_pos = build_add_left_pos_fn(c, &b, m.clone());
...
impl Environment {
    /// Register `Int.add_assoc_ofNat_succ_negSucc_ofNat_succ` as a kernel-checked theorem.
    pub(crate) fn register_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_proof(
        &mut self,
    ) -> Result<(), EnvError> {
        let name = Name::from_string("Int.add_assoc_ofNat_succ_negSucc_ofNat_succ");
        ...
        let c = IntAddAssocOfNatSuccNegSuccOfNatSuccConsts::new();
        let type_ = build_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_type(&c);
        let value = build_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_value(&c);
...
mod tests {
    ...
    #[test]
    fn test_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_registered_as_theorem() {
        let mut env = Environment::new();
        env.register_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_proof()
        ...
        let info = env
            ...
            .expect("Int.add_assoc_ofNat_succ_negSucc_ofNat_succ should be registered");
        assert_eq!(info.kind, ConstantKind::Theorem);
        assert!(info.value.is_some(), "Theorem must retain its proof value");
    }

    #[test]
    fn test_int_add_assoc_ofnat_succ_negsucc_ofnat_succ_axiom_closure_is_empty() {
        let mut env = Environment::new();
        ...
        assert!(
            ...
            "Int.add_assoc_ofNat_succ_negSucc_ofNat_succ must be Constructive, got {:?}",
            quality
        );
    }
}
