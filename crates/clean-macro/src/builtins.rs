...
/// Built-in syntax kinds for macro expansion.
impl SyntaxKind {
    /// Syntax kind for `if cond then t else f` expressions.
    pub fn if_then_else() -> Self {
        Self::app("ifThenElse")
    }
    /// Syntax kind for `if let` pattern matching.
    pub fn if_let() -> Self {
        Self::app("ifLet")
    }
    /// Syntax kind for `do` notation blocks.
    pub fn do_notation() -> Self {
        Self::app("do")
    }
    /// Syntax kind for individual elements in `do` blocks.
    pub fn do_elem() -> Self {
        Self::app("doElem")
    }
    /// Syntax kind for `let` bindings in `do` blocks.
    pub fn do_let() -> Self {
        Self::app("doLet")
    }
    /// Syntax kind for monadic bind (`<-`) in `do` blocks.
    pub fn do_bind() -> Self {
        Self::app("doBind")
    }
    /// Syntax kind for `return` in `do` blocks.
    pub fn do_return() -> Self {
        Self::app("doReturn")
    }
    /// Syntax kind for `unless` conditional.
    pub fn unless() -> Self {
        Self::app("unless")
    }
    /// Syntax kind for `when` conditional.
    pub fn when() -> Self {
        Self::app("when")
    }
    /// Syntax kind for `dbg_trace` debugging macro.
    pub fn dbg_trace() -> Self {
        Self::app("dbgTrace")
    }
    /// Syntax kind for `assert!` macro.
    pub fn assert_macro() -> Self {
        Self::app("assert")
    }
    /// Syntax kind for `panic!` macro.
    pub fn panic_macro() -> Self {
        Self::app("panic")
    }
    /// Syntax kind for `have` tactic/term.
    pub fn have_macro() -> Self {
        Self::app("haveMacro")
    }
    /// Syntax kind for `let` tactic/term.
    pub fn let_macro() -> Self {
        Self::app("letMacro")
    }
    /// Syntax kind for `show` tactic/term.
    pub fn show_macro() -> Self {
        Self::app("showMacro")
    }
    /// Syntax kind for `match` expressions.
    pub fn match_expr() -> Self {
        Self::app("match")
    }
    /// Syntax kind for individual match arms.
    pub fn match_arm() -> Self {
        Self::app("matchArm")
    }
    /// Syntax kind for `calc` proofs.
    pub fn calc_macro() -> Self {
        Self::app("calc")
    }
    /// Syntax kind for `conv` tactic mode.
...
fn register_do_notation(registry: &mut MacroRegistry) {
    ...
    // N-element do notation (3+ elements): recursive expansion
    //
    // Pattern: (do $first $[rest]*)
    // Result: expand $first handling, wrap rest in do and recurse
    //
    // For do { e1; e2; rest... }, we need to:
    // 1. Process first element (e1) based on its kind
    // 2. Recursively wrap remaining elements in do
    //
    // The key insight: since we already have 2-element patterns with priority 5-10,
    // we add a lower-priority pattern that matches "first + splice of 2+" and
    // rewrites to "processed-first + do { splice }".

    // do { e1; rest... } (3+ elements via bind) => bind e1 (fun x => do { rest... })
    // Pattern: (do (doBind $x $e1) $e2 $[rest]*)
    // The presence of $e2 ensures at least 3 elements total
    let do_bind_n_def = MacroDef::new(
        "doBindN",
        SyntaxKind::do_notation(),
        Syntax::node(
            SyntaxKind::do_notation(),
            vec![
                ...
                Syntax::mk_antiquot("e2"),
                Syntax::mk_antiquot_splice("rest"),
            ],
        ),
        SyntaxQuote::term(Syntax::mk_app(
            Syntax::mk_app(Syntax::ident("bind"), vec![Syntax::mk_antiquot("e1")]),
            vec![Syntax::mk_lambda(
                vec![Syntax::mk_antiquot("x")],
                // Recursively wrap remaining elements in do
                Syntax::node(
                    SyntaxKind::do_notation(),
                    vec![
                        Syntax::mk_antiquot("e2"),
    ...
    .with_priority(3) // Lower than 2-element patterns (5-10)
    .with_doc("do { x <- e1; e2; rest... } => bind e1 (fun x => do { e2; rest... })");
    registry.register(do_bind_n_def);

    // do { let x := e1; rest... } (3+ elements) => let x := e1 in do { rest... }
    let do_let_n_def = MacroDef::new(
        "doLetN",
        SyntaxKind::do_notation(),
        Syntax::node(
            SyntaxKind::do_notation(),
    ...
    .with_priority(3)
    .with_doc("do { let x := e1; e2; rest... } => let x := e1 in do { e2; rest... }");
    registry.register(do_let_n_def);

    // do { e1; rest... } (3+ elements via seq) => seq e1 (fun _ => do { rest... })
    let do_seq_n_def = MacroDef::new(
        "doSeqN",
        SyntaxKind::do_notation(),
        Syntax::node(
            SyntaxKind::do_notation(),
    ...
    .with_priority(2) // Even lower than bind/let N-element
    .with_doc("do { e1; e2; rest... } => seq e1 (fun _ => do { e2; rest... })");
    registry.register(do_seq_n_def);
}
...
/// Additional syntax kinds for calc and conv macros.
impl SyntaxKind {
    /// Syntax kind for individual steps in `calc` proofs.
    pub fn calc_step() -> Self {
        Self::app("calcStep")
    }
    /// Syntax kind for individual steps in `conv` mode.
    pub fn conv_step() -> Self {
        Self::app("convStep")
    }
    /// Syntax kind for sequences of `conv` tactics.
...
fn register_calc_conv(registry: &mut MacroRegistry) {
    ...
    // N-step calc (4+ steps): recursive expansion
    //
    // Pattern: (calc (calcStep $h1) (calcStep $h2) $[rest]*)
    // where rest contains at least 2 more steps (for 4+ total)
    //
    // Strategy: combine first two steps, then wrap with remaining steps
    // calc h1; h2; h3; rest... => calc (Trans.trans h1 h2); h3; rest...
    //
    // However, this changes the result type. Better approach:
    // calc h1; h2; h3; rest... => Trans.trans h1 (calc h2; h3; rest...)
    //
    // Pattern: (calc (calcStep $h1) (calcStep $h2) (calcStep $h3) $[rest]*)
    let calc_n_step = MacroDef::new(
        "calcNStep",
        SyntaxKind::calc_macro(),
        Syntax::node(
            SyntaxKind::calc_macro(),
    ...
    .with_priority(5) // Lower than specific-count patterns
    .with_doc("calc with 4+ steps => Trans.trans h1 (calc h2; h3; rest...)");
    registry.register(calc_n_step);
...
mod tests {
    ...
    fn test_builtin_registry_creation() {
        ...
        assert!(
            registry.get_by_name("ifThenElse").is_some(),
            "builtin registry should contain ifThenElse"
        );
        assert!(
            registry.get_by_name("unless").is_some(),
            "builtin registry should contain unless"
        );
        assert!(
            registry.get_by_name("when").is_some(),
            "builtin registry should contain when"
        );
    }
    ...
    #[test]
    fn test_do_seq_n_expansion() {
        let registry = builtin_registry();

        // do { e1; e2; e3 } => seq e1 (fun _ => do { e2; e3 })
        // which then expands further to: seq e1 (fun _ => seq e2 (fun _ => e3))
        let input = Syntax::node(
            SyntaxKind::do_notation(),
            vec![
                Syntax::node(SyntaxKind::do_elem(), vec![Syntax::ident("action1")]),
                Syntax::node(SyntaxKind::do_elem(), vec![Syntax::ident("action2")]),
                Syntax::node(SyntaxKind::do_elem(), vec![Syntax::ident("action3")]),
            ],
        );
        ...
        let pretty = result.pretty();
        // Should have nested seq calls
        assert!(pretty.contains("seq"));
        assert!(pretty.contains("fun"));
        // Should contain all three actions
        assert!(pretty.contains("action1"));
        assert!(pretty.contains("action2"));
        assert!(pretty.contains("action3"));
    }

    #[test]
    fn test_do_bind_n_expansion() {
        let registry = builtin_registry();

        // do { x <- e1; y <- e2; e3 } should use bind recursively
        let input = Syntax::node(
            SyntaxKind::do_notation(),
            vec![
                Syntax::node(
                ...
                Syntax::node(
                    SyntaxKind::do_bind(),
                    vec![Syntax::ident("y"), Syntax::ident("getLine")],
                ),
                Syntax::node(
                    SyntaxKind::do_elem(),
                    vec![Syntax::mk_app(
        ...
        let pretty = result.pretty();
        // Should have nested bind calls
        assert!(pretty.contains("bind"));
        assert!(pretty.contains("fun"));
        // Should contain bound variables
        assert!(pretty.contains("getLine"));
    }
    ...
    #[test]
    fn test_calc_four_step() {
        let registry = builtin_registry();

        // calc with four steps => Trans.trans h1 (calc h2; h3; h4)
        // which then recursively expands
        let input = Syntax::node(
            SyntaxKind::calc_macro(),
            vec![
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h1")]),
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h2")]),
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h3")]),
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h4")]),
            ],
        );

        let result = expand(&registry, input).unwrap();
        let pretty = result.pretty();
        // Should have nested Trans.trans calls
        assert!(pretty.contains("Trans.trans"));
        // Should contain all proof steps
        assert!(pretty.contains("h1"));
        assert!(pretty.contains("h2"));
        assert!(pretty.contains("h3"));
        assert!(pretty.contains("h4"));
    }

    #[test]
    fn test_calc_five_step() {
        let registry = builtin_registry();

        // calc with five steps - tests full recursion
        let input = Syntax::node(
            SyntaxKind::calc_macro(),
            vec![
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h1")]),
                ...
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h3")]),
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h4")]),
                Syntax::node(SyntaxKind::calc_step(), vec![Syntax::ident("h5")]),
            ],
        );
