...
/// Run a resource-sensitive test body in a fresh copy of the current libtest
/// binary.
///
/// AY's production embedded limiter intentionally measures whole-process RSS.
/// That is the right fail-closed contract for a compiler host, but a libtest
/// process accumulates allocator-retained memory from thousands of unrelated
/// tests. A tiny solve late in the suite can therefore inherit another test's
/// RSS and return `Unknown(MemoryLimit)`. The child retains the exact production
/// limiter while giving this test its own process-level resource account.
pub(crate) fn in_isolated_test_process(body: impl FnOnce()) {
    const WORKER_ENV: &str = "CLEAN_AUTO_ISOLATED_TEST_WORKER";
    ...
    let test_name = current_thread
        .name()
        .expect("libtest worker threads have the fully qualified test name");
    let executable = std::env::current_exe().expect("resolve current libtest executable");
    let output = std::process::Command::new(executable)
        .args(["--exact", test_name, "--nocapture", "--test-threads=1"])
        .env(WORKER_ENV, "1")
        .output()
        .expect("spawn isolated libtest worker");

    assert!(
        output.status.success(),
        "isolated test worker `{test_name}` failed with {}\nstdout:\n{}\nstderr:\n{}",
        output.status,
