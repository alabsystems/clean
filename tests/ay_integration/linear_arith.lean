/-
  Ay Integration Tests: Linear Integer Arithmetic (QF_LIA)

  Tests the ay_omega tactic for linear integer arithmetic goals.
-/
...
theorem square_nonneg (x : Int) : x * x >= 0 := by
  ay_omega  -- May need nonlinear extension
