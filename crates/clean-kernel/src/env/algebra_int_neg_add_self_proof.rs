...
//! `Int.neg_add_self : ∀ a : Int, Eq Int (Int.add (Int.neg a) a) Int.zero`.
//!
...
//! left additive-inverse identity `(-a) + a = 0`.
//!
//! # Proof sketch
//!
...
//! `λ x : Int => Eq Int (Int.add (Int.neg x) x) (Int.ofNat 0)`:
//!
//! - **`ofNat n` branch**: recurse on `n` with an inner `@Nat.rec.{0}`:
//!   - `zero`: `Int.add (Int.neg (ofNat 0)) (ofNat 0) = Int.add (ofNat 0)
//!     (ofNat 0) = ofNat 0`. Closed by `@Eq.refl.{1} Int (ofNat 0)`.
//!   - `succ k`: `Int.neg (ofNat (succ k)) = negSucc k`, then
//!     `Int.add (negSucc k) (ofNat (succ k)) = subNatNat (succ k) (succ k)`.
//!     Closed by `Int.subNatNat_self (Nat.succ k)`.
//!
//! - **`negSucc m` branch**: `Int.neg (negSucc m) = ofNat (succ m)`, then
//!   `Int.add (ofNat (succ m)) (negSucc m) = subNatNat (succ m) (succ m)`.
...
//! `Declaration::Axiom`, so `env.axiom_deps("Int.neg_add_self")` is empty and
//! `env.proof_quality("Int.neg_add_self") == ProofQuality::Constructive`.
//!
//! Tracks #3604. Sibling proofs:
//! - `algebra_int_sub_nat_nat_self_proof.rs` (dependency).
//! - `algebra_int_add_neg_self_proof.rs` (companion — Int.add_neg_self).
...
impl IntNegAddSelfConsts {
    ...
    fn neg_add(&self, x: Expr) -> Expr {
        self.add(self.neg(x.clone()), x)
    }
...
/// Build `∀ a : Int, Eq Int (Int.add (Int.neg a) a) Int.zero`.
fn build_type(c: &IntNegAddSelfConsts) -> Expr {
    let mut b = EnvDeclBuilder::new();
    let (a_id, a) = b.fresh_local(c.int_type.clone());
    let concl = c.eq_int(c.neg_add(a), c.zero_int());
    let ty_raw = b.mk_pi(a_id, BinderInfo::Default, c.int_type.clone(), concl);
    b.finish(ty_raw)
}

/// Outer motive: `λ (x : Int) => Eq Int (Int.add (Int.neg x) x) (Int.ofNat 0)`.
fn build_outer_motive(c: &IntNegAddSelfConsts, parent: &EnvDeclBuilder) -> Expr {
    let mut mb = EnvDeclBuilder::child_of(parent);
    let (x_id, x) = mb.fresh_local(c.int_type.clone());
    let body = c.eq_int(c.neg_add(x), c.zero_int());
...
fn build_ofnat_case(c: &IntNegAddSelfConsts, parent: &EnvDeclBuilder) -> Expr {
    let mut ob = EnvDeclBuilder::child_of(parent);
    let (n_id, n) = ob.fresh_local(c.nat_type.clone());

    // inner motive:
    //   λ (t : Nat) => Eq Int (Int.add (Int.neg (ofNat t)) (ofNat t)) (ofNat 0)
    let inner_motive = {
        let mut mb = EnvDeclBuilder::child_of(&ob);
        let (t_id, t) = mb.fresh_local(c.nat_type.clone());
        let body = c.eq_int(c.neg_add(c.of_nat(t)), c.zero_int());
    ...
    let succ_case = {
        ...
        let ih_type = c.eq_int(c.neg_add(c.of_nat(k.clone())), c.zero_int());
...
fn build_negsucc_case(c: &IntNegAddSelfConsts, parent: &EnvDeclBuilder) -> Expr {
...
fn build_value(c: &IntNegAddSelfConsts) -> Expr {
...
impl Environment {
    /// Register `Int.neg_add_self` as a kernel-checked `Declaration::Theorem`.
    ///
    /// # Contract
    ///
    ...
    /// ENSURES: On success, `Int.neg_add_self` is a `Declaration::Theorem`
    ///          with `proof_quality == Constructive`.
    /// ENSURES: Idempotent — if `Int.neg_add_self` is already registered with
    ///          any declaration kind, this call returns `Ok(())` without
    ///          modification.
    pub(crate) fn register_int_neg_add_self_proof(&mut self) -> Result<(), EnvError> {
        ...
        let name = Name::from_string("Int.neg_add_self");
        ...
        // `Int.subNatNat_self (succ m)`. The kernel reduces `Int.add
        // (Int.neg x) x` to `Int.subNatNat (succ ·) (succ ·)` on the non-zero
...
mod tests {
    ...
    #[test]
    fn test_int_neg_add_self_registered_as_theorem() {
        let mut env = Environment::new();
        env.register_int_neg_add_self_proof()
            .expect("first registration");
        env.register_int_neg_add_self_proof()
            .expect("idempotent re-registration");
        let info = env
            .get_const(&Name::from_string("Int.neg_add_self"))
            .expect("Int.neg_add_self should be registered");
    ...
    #[test]
    fn test_int_neg_add_self_proof_uses_int_rec() {
        use crate::expr::ExprKind;
        let mut env = Environment::new();
        env.register_int_neg_add_self_proof().unwrap();
        let info = env
            .get_const(&Name::from_string("Int.neg_add_self"))
            .expect("registered");
        ...
        match head.kind() {
            ExprKind::Const(n, _) => assert_eq!(
                n.to_string(),
                "Int.rec",
                "Int.neg_add_self proof root must be Int.rec, got Const({:?})",
                n.to_string()
            ),
    ...
    #[test]
    fn test_int_neg_add_self_axiom_deps_empty() {
        let mut env = Environment::new();
        env.register_int_neg_add_self_proof().unwrap();
        let deps = env
            .axiom_deps(&Name::from_string("Int.neg_add_self"))
            .expect("Int.neg_add_self is registered, axiom_deps should return Some");
        let domain_deps: Vec<String> = deps.iter().map(|n| n.to_string()).collect();
        assert!(
            domain_deps.is_empty(),
            "Int.neg_add_self must have empty axiom closure (constructive proof), got {:?}",
            domain_deps
        );
    }
}
