...
//! KKL real/cbrt layer (ported from the sqrt layer; cbrtDyadicApprox mirrors dyadicApprox) — the per-step UPPER bound `a_{n+1} ≤ a_n + inv(2^{n+1})`
...
//! `Rat.cbrtDyadicApprox_le_succ`) this pins the increment into `[0, inv(2^{n+1})]`.
//!
...
//! - `Rat.cbrtDyadicApprox_succ_le : ∀ x n,
//!       Rat.le (Rat.cbrtDyadicApprox x (Nat.succ n))
//!              (Rat.add (Rat.cbrtDyadicApprox x n) (inv (ofNat 2^{n+1})))`.
//!
...
pub(crate) struct CbrtStepConsts {
    nat: Expr,
    nat_zero: Expr,
    nat_succ: Expr,
    nat_mul: Expr,
...
impl CbrtStepConsts {
    pub(crate) fn new() -> Self {
        ...
        Self {
            ...
            rat_dyadic_num_succ_le_two_mul_succ: k("Rat.cbrtDyadicNum_succ_le_two_mul_succ"),
...
impl Environment {
    ...
    pub fn init_algebra_nnreal_cbrt_cauchy_step(&mut self) -> Result<(), EnvError> {
        self.init_eq()?;
        self.init_and()?;
        self.init_iff()?;
        self.init_nat()?;
        self.init_algebra_nnreal_cbrt_seq()?; // dyadicApprox, dyadicNum, ofNat
        self.init_algebra_nnreal_cbrt_mono()?; // dyadicNum_succ_le_two_mul_succ
        ...
        let c = CbrtStepConsts::new();
        self.register_cbrt_ofnat_two_mul_inv_two_pow_succ(&c)?;
        self.register_cbrt_dyadic_approx_succ_le(&c)?;
        Ok(())
    }
    ...
    fn register_cbrt_ofnat_two_mul_inv_two_pow_succ(
        &mut self,
        c: &CbrtStepConsts,
    ) -> Result<(), EnvError> {
    ...
    fn register_cbrt_dyadic_approx_succ_le(&mut self, c: &CbrtStepConsts) -> Result<(), EnvError> {
        let name = Name::from_string("Rat.cbrtDyadicApprox_succ_le");
        if self.get_const(&name).is_some() {
            return Ok(());
        }
        let two = c.nat_lit(2);
...
mod tests {
    ...
    fn env() -> Environment {
        ...
        env.init_algebra_nnreal_cbrt_cauchy_step()
            .expect("init_algebra_nnreal_cbrt_cauchy_step");
        env.init_algebra_nnreal_cbrt_cauchy_step()
            .expect("idempotent");
        env
    }
