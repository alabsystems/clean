...
//! Tier A axiom-reject recovery (#3551): `NNVerify.Rat.mul_one_zero`.
//!
//! Instantiates `Rat.mul_zero : ∀ a : Rat, a * 0 = 0` at `a = Rat.one`.
...
//!
//! - `NNVerify.Rat.mul_one_zero : Rat.mul Rat.one Rat.zero = Rat.zero`
//!
//!   Proof term: `Rat.mul_zero Rat.one`.
//!
//! ## Part of
//!
...
impl Environment {
    /// Initialize the Tier A `Rat.mul_one_zero` lemma (#3551 Batch 3).
    ///
    /// # Contract
    ///
    /// REQUIRES: `self` is a valid Environment
    /// ENSURES: On success, `self.nn_verify_tier_a_rat_mul_one_zero_init == true`
    /// ENSURES: Idempotent
    pub fn init_nn_verify_tier_a_rat_mul_one_zero(&mut self) -> Result<(), EnvError> {
        if self.nn_verify_tier_a_rat_mul_one_zero_init {
            return Ok(());
        }
        self.init_rat_field_inst()?; // Rat.mul_zero (foundational axiom)
        self.init_eq()?;

        self.register_rat_mul_one_zero()?;

        self.nn_verify_tier_a_rat_mul_one_zero_init = true;
        Ok(())
    }

    /// `NNVerify.Rat.mul_one_zero : Rat.mul Rat.one Rat.zero = Rat.zero`.
    ///
    /// Proof term: `Rat.mul_zero Rat.one`. Transitive non-foundational
    /// axiom closure is empty — `Rat.mul_zero` is in `FOUNDATIONAL_AXIOMS`.
    fn register_rat_mul_one_zero(&mut self) -> Result<(), EnvError> {
        let name = Name::from_string("NNVerify.Rat.mul_one_zero");
        ...
        // Type: Rat.mul Rat.one Rat.zero = Rat.zero
        let lhs = Expr::apps(rat_mul, [rat_one.clone(), rat_zero.clone()]);
        let ty = Expr::apps(eq, [rat, lhs, rat_zero]);

        // Proof term: Rat.mul_zero Rat.one
        let mul_zero = Expr::const_(Name::from_string("Rat.mul_zero"), vec![]);
        let value = Expr::app(mul_zero, rat_one);
