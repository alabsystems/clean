...
pub struct FuncDef {
    ...
    pub storage: StorageClass,
    /// `_Noreturn` function specifier (C11 6.7.4) — also the
    /// `<stdnoreturn.h>` `noreturn` convenience macro. A `_Noreturn`
    /// function must never return to its caller; doing so is undefined
    /// behavior (C11 6.7.4p2).
...
impl FuncDef {
    ...
    /// Mark this function as `_Noreturn` (C11 6.7.4).
    #[must_use]
    pub fn with_noreturn(mut self, is_noreturn: bool) -> Self {
...
pub enum CStmt {
    ...
    /// Static assertion (C11 6.7.10): `_Static_assert(const-expr, "message");`
    /// or the C23 single-argument form `_Static_assert(const-expr);`.
    ///
    /// The constant expression is checked at compile time: if it evaluates to
    /// zero the program is ill-formed and a diagnostic carrying `message` is
    /// emitted; a non-zero value makes this a no-op at run time.
    StaticAssert {
        /// The controlling constant expression (must be non-zero).
        cond: CExpr,
        /// The diagnostic message (absent in the C23 single-argument form).
...
impl CStmt {
    ...
    /// Create a static assertion statement
    pub fn static_assert(cond: CExpr, message: Option<String>) -> Self {
...
mod tests {
    ...
    fn test_var_decl() {
        ...
        assert!(decl.init.is_some(), "decl with_init should have init");
    }
    ...
    fn test_if_stmt() {
        ...
        match stmt {
            CStmt::If { else_stmt, .. } => {
                assert!(else_stmt.is_some(), "if_else should have else branch")
            }
    ...
    fn test_for_loop() {
        ...
        match stmt {
            CStmt::For {
                ...
                assert!(init.is_some(), "for_loop should have init");
                assert!(cond.is_some(), "for_loop should have cond");
                assert!(update.is_some(), "for_loop should have update");
            }
    ...
    fn test_translation_unit() {
        ...
        assert!(
            tu.find_func("main").is_some(),
            "should find 'main' function"
        );
        assert!(
            tu.find_global("global_x").is_some(),
            "should find 'global_x' variable"
        );
