...
//! COMMITTED end-to-end LINK+RUN census of the Clean producer over the FULL
//! computable prelude (`Environment::with_prelude` + `init_io_ops`), the
//! measurement instrument for the "every computable prelude definition must
//! lower AND link AND run as a binary — 100%, no exceptions" goal.
//!
...
//!      wrong value is `RUN_FAIL`, never `OK`.
//!
//! FAIL-CLOSED: a root is `OK` only if it actually emits, links, and its binary
//! executes (exit 0) — and, where checkable, computes the CORRECT value. The
//! 28 embedded shims are a verbatim mirror of `clean-cli/native_build.rs`
//! (origin/main 304e0cb23); the denylist is the union of their symbols. The
//! shim set is confirmed against the real `clean run` in the report cross-check.
//!
//! This is intentionally `#[ignore]`d (it forks `cc` ~800×). Run:
//!   LINKRUN_OUT=/path/out.tsv cargo test -p clean-compiler --release \
//!     --features round-trip-compile --test linkrun_census -- --ignored --nocapture
//!
//! Optional env: `LINKRUN_LIMIT=N` (first N computable roots, for iteration);
//! `LINKRUN_NO_IO=1` (skip `init_io_ops`).
...
fn linkrun_census() {
    let Some(cc) = find_c_compiler() else {
        eprintln!("linkrun_census skipped: no C compiler (cc/gcc/clang)");
        return;
    };
    let limit: Option<usize> = std::env::var("LINKRUN_LIMIT")
        .ok()
        .and_then(|s| s.parse().ok());
    let out_path = std::env::var("LINKRUN_OUT").ok();

    let mut env = Environment::with_prelude();
    if std::env::var("LINKRUN_NO_IO").is_err() {
    ...
    println!("COMPUTABLE defs (population) = {total}");
    println!("-------------------------------------------------------");
    println!("RUNNABLE OK                 = {n_ok} / {total}");
    for (tag, c) in &counts {
        println!("  {tag:<12} = {c}");
    }
    println!("=======================================================");
    ...
    if let Some(path) = out_path {
        ...
        writeln!(
            f,
            "# total_constants={} non_computable={} lower_err={} theorems={} opaque={} axiomkind={} computable_defs={} ok={}",
            names.len(), n_none, n_lowfail, n_theorem, n_opaque, n_axiomkind, total, n_ok
        ).unwrap();
    ...
    // The instrument itself must not regress into vacuity.
