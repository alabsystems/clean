...
pub(super) fn lake_build(
    ...
    let options = BuildOptions::new()
        ...
        .with_force(force)
        .with_permissive_imports(permissive_imports);
...
pub(super) fn lake_new(name: &str, _lib: bool, exe: bool) -> anyhow::Result<()> {
    ...
    fs::write(
        project_dir.join("lakefile.lean"),
        new_project_lakefile(project_name, exe),
    )?;
...
pub(super) fn lake_init(name: Option<String>, dir: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    // Generate lakefile.lean
    fs::write(
        cwd.join("lakefile.lean"),
        init_project_lakefile(&package_name, &module_name, &test_name),
    )?;
    fs::write(
...
/// The `lakefile.lean` template emitted by `clean lake new`.
///
/// Kept as a function so the strict-parse regression tests can assert the
/// template round-trips through `LakeConfig::parse` with zero
/// skipped-construct diagnostics.
fn new_project_lakefile(project_name: &str, exe: bool) -> String {
    if exe {
        format!(
            r#"import Lake
open Lake DSL
...
/// The `lakefile.lean` template emitted by `clean lake init`.
fn init_project_lakefile(package_name: &str, module_name: &str, test_name: &str) -> String {
    format!(
        r#"import Lake
open Lake DSL
...
#[cfg(test)]
mod lakefile_template_tests {
    use super::*;
    use clean_lake::{LakeConfig, LakefileParseMode};

    #[test]
    fn test_lake_new_templates_strict_parse_with_zero_diagnostics() {
        for exe in [false, true] {
            let content = new_project_lakefile("demo", exe);
            // Strict mode fails on ANY unrecognized top-level construct, so a
            // successful strict parse certifies zero diagnostics.
            let config = LakeConfig::parse_with_mode(&content, LakefileParseMode::Strict)
                .expect("clean lake new template must strict-parse");
            assert!(
                config.diagnostics.is_empty(),
                "lake new template (exe={exe}) must carry zero skipped-construct \
                 diagnostics: {:?}",
                config.diagnostics
            );
    ...
    #[test]
    fn test_lake_init_template_strict_parses_with_zero_diagnostics() {
        let content = init_project_lakefile("demo", "Demo", "demo_test");
        let config = LakeConfig::parse_with_mode(&content, LakefileParseMode::Strict)
            .expect("clean lake init template must strict-parse");
        assert!(
            config.diagnostics.is_empty(),
            "lake init template must carry zero skipped-construct diagnostics: {:?}",
            config.diagnostics
        );
        assert_eq!(config.package.name, "demo");
        assert_eq!(config.tests.len(), 1, "init template declares a lean_test");
    }
}
