/-
  Ay Integration Tests: Performance Benchmarks

  Benchmark problems to verify performance targets are met.
  All times are wall-clock, measured on CI machines.
-/

import clean.Tactic.Ay

-- =============================================================================
-- Target Latencies (from Ay_INTEGRATION_RESPONSE.md)
-- =============================================================================
...
-- Failure = regression in Ay/clean integration
...
theorem perf_regression_bv : True := by
  -- Must complete in < 10ms
  have h : ∀ (x : UInt32), x * 0 = 0 := by ay_bv (timeout := 10)
  trivial
