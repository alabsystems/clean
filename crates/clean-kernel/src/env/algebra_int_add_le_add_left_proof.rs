...
//! `Int.le (c + a) (c + b)` to `Int.NonNeg (Int.sub (c + b) (c + a))`.
//!
//! # Proof sketch
//!
//! `Int.add_sub_add_left a b c : Eq Int (Int.sub (c+b) (c+a)) (Int.sub b a)`
//! (`(c+b) - (c+a) = b - a`). Transport `h : NonNeg (Int.sub b a)` along its
//! `Eq.symm` with motive `fun x => Int.NonNeg x`:
//!
//! ```text
//! @Eq.subst.{1} Int (fun x => Int.NonNeg x)
//!   (Int.sub b a) (Int.sub (c+b) (c+a))
//!   (@Eq.symm.{1} Int (Int.sub (c+b) (c+a)) (Int.sub b a) (Int.add_sub_add_left a b c))
//!   h
//!   : Int.NonNeg (Int.sub (c+b) (c+a))   ≡   Int.le (c + a) (c + b)
//! ```
//!
//! # Axiom closure
//!
...
//! `Declaration::Axiom`, so `env.axiom_deps("Int.add_le_add_left")` is empty and
//! `env.proof_quality("Int.add_le_add_left") == ProofQuality::Constructive`.
...
/// Build `∀ a b : Int, Int.le a b → ∀ c : Int, Int.le (c+a) (c+b)`.
fn build_type(c: &IntAddLeAddLeftConsts) -> Expr {
    ...
    let concl = c.le(c.add(cc.clone(), a.clone()), c.add(cc.clone(), bv.clone()));
...
///     h
/// ```
fn build_value(c: &IntAddLeAddLeftConsts) -> Expr {
    let mut b = EnvDeclBuilder::new();
    let (a_id, a) = b.fresh_local(c.int_type.clone());
    let (b_id, bv) = b.fresh_local(c.int_type.clone());
    let le_ab = c.le(a.clone(), bv.clone());
    ...
    let sub_cbca = c.sub(c.add(cc.clone(), bv.clone()), c.add(cc.clone(), a.clone())); // (c+b)-(c+a)

    // Int.add_sub_add_left a b c : Eq Int ((c+b)-(c+a)) (b-a)
    let id_eq = Expr::apps(
        c.add_sub_add_left.clone(),
        [a.clone(), bv.clone(), cc.clone()],
    );
    // @Eq.symm.{1} Int ((c+b)-(c+a)) (b-a) id_eq : Eq Int (b-a) ((c+b)-(c+a))
    let symm = Expr::apps(
        c.eq_symm.clone(),
        [c.int_type.clone(), sub_cbca.clone(), sub_ba.clone(), id_eq],
    );

    // @Eq.subst.{1} Int motive (b-a) ((c+b)-(c+a)) symm h : NonNeg ((c+b)-(c+a))
    let proof = Expr::apps(
        c.eq_subst.clone(),
        [
            c.int_type.clone(),
...
impl Environment {
    /// Register `Int.add_le_add_left` as a kernel-checked `Declaration::Theorem`.
    ///
    /// # Contract
    ///
    ...
    /// ENSURES: On success, `Int.add_le_add_left` is a `Declaration::Theorem`
    ///          with `proof_quality == Constructive`.
    /// ENSURES: Idempotent — if `Int.add_le_add_left` is already registered with
    ///          any declaration kind, this call returns `Ok(())` without
    ///          modification.
    pub(crate) fn register_int_add_le_add_left_proof(&mut self) -> Result<(), EnvError> {
        ...
        let name = Name::from_string("Int.add_le_add_left");
...
mod tests {
    ...
    #[test]
    fn test_int_add_le_add_left_registered_as_theorem() {
        let mut env = Environment::new();
        env.register_int_add_le_add_left_proof()
            .expect("first registration");
        env.register_int_add_le_add_left_proof()
            .expect("idempotent re-registration");
        let info = env
            .get_const(&Name::from_string("Int.add_le_add_left"))
            .expect("Int.add_le_add_left should be registered");
        assert_eq!(info.kind, ConstantKind::Theorem);
        assert!(info.value.is_some(), "Theorem must retain its proof value");
    }

    #[test]
    fn test_int_add_le_add_left_proof_body_uses_eq_subst() {
        use crate::expr::ExprKind;
        let mut env = Environment::new();
        env.register_int_add_le_add_left_proof().unwrap();
        let info = env
            .get_const(&Name::from_string("Int.add_le_add_left"))
            .expect("registered");
        ...
        match head.kind() {
            ExprKind::Const(n, _) => assert_eq!(
                n.to_string(),
                "Eq.subst",
                "Int.add_le_add_left proof root must be Eq.subst"
            ),
    ...
    #[test]
    fn test_int_add_le_add_left_axiom_deps_empty() {
        let mut env = Environment::new();
        env.register_int_add_le_add_left_proof().unwrap();
        let deps = env
            .axiom_deps(&Name::from_string("Int.add_le_add_left"))
            .expect("Int.add_le_add_left is registered, axiom_deps should return Some");
        let domain_deps: Vec<String> = deps.iter().map(|n| n.to_string()).collect();
        assert!(
            domain_deps.is_empty(),
            "Int.add_le_add_left must have empty axiom closure (constructive proof), got {:?}",
            domain_deps
        );
    }

    #[test]
    fn test_int_add_le_add_left_proof_quality_constructive() {
        use crate::env::ProofQuality;
        let mut env = Environment::new();
        env.register_int_add_le_add_left_proof().unwrap();
        let quality = env
            .proof_quality(&Name::from_string("Int.add_le_add_left"))
            .expect("proof_quality should be reported");
        assert!(
            matches!(quality, ProofQuality::Constructive),
            "Int.add_le_add_left must be Constructive, got {:?}",
            quality
        );
    }
}
