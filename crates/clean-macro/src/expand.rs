...
impl<'a> HygienicExpander<'a> {
    ...
    fn expand_inner(&mut self, syntax: Syntax) -> MacroResult<Syntax> {
        ...
        // Determine whether a macro matches before mutating any state.
        let matched = self.registry.try_expand(&syntax).is_some();
        ...
        let result = if matched {
            ...
            // Push a new scope for this macro expansion BEFORE applying the
            // macro, so a computed body's fresh-name markers gensym inside this
            // expansion's own scope (distinct per application). Disjoint field
            // borrows let `try_expand_hygienic` read the registry while mutating
            // the hygiene state.
            let _scope = self.hygiene.state_mut().push_scope();

            // Apply the macro hygienically: fresh-name markers in the template
            // are gensym'd here (fresh per expansion); static templates expand
            // exactly as the non-hygienic path. We re-run the match under the
            // active scope rather than reuse the probe result above so the
            // gensym counter is advanced inside this scope.
            let expanded = self
                .registry
                .try_expand_hygienic(&syntax, self.hygiene.state_mut())
                .unwrap_or(syntax);
...
mod tests {
    ...
    #[test]
    fn test_fresh_marker_distinct_id_per_expansion() {
        // A computed-body template carrying a fresh-name marker — modelling
        // `macro_rules | `(dup) => do let f <- mkFreshId; return `(fun $f => $f)`.
        // The template is the SAME stored `MacroDef.replacement` on every
        // application; the fix is that `mkFreshId`'s id must be gensym'd PER
        // expansion. We expand `dup` TWICE and require the two fresh binders to
        // DIFFER (before the fix they would be byte-identical).
        let mut registry = MacroRegistry::new();
        let kind = SyntaxKind::app("dup");
        // Template: (fun <fresh f> <fresh f>) — both markers share prefix "f"
        // so within ONE expansion they resolve to the same name (binder == use),
        // while distinct expansions get distinct names.
        let template = Syntax::node(
            SyntaxKind::fun(),
            vec![Syntax::mk_fresh_marker("f"), Syntax::mk_fresh_marker("f")],
        );
        ...
        let mut expander = HygienicExpander::new(&registry);
        let first = expander
            .expand(Syntax::node(kind.clone(), vec![]))
            .expect("first dup expansion");
        let second = expander
            .expand(Syntax::node(kind, vec![]))
            .expect("second dup expansion");

        // Within each expansion, the binder name equals its use (same prefix).
        let b1 = first.child(0).unwrap().as_ident().unwrap();
        let u1 = first.child(1).unwrap().as_ident().unwrap();
        assert_eq!(b1, u1, "binder and use must share the one fresh id");
        let b2 = second.child(0).unwrap().as_ident().unwrap();
        let u2 = second.child(1).unwrap().as_ident().unwrap();
        assert_eq!(b2, u2, "binder and use must share the one fresh id");

        // Across expansions, the fresh ids must DIFFER — the per-expansion fix.
        assert_ne!(
            b1, b2,
            "two expansions of a fresh-name macro must yield distinct fresh ids"
        );
    }

    #[test]
    fn test_static_template_unaffected_by_hygienic_path() {
        // NEGATIVE: a template with NO fresh markers expands byte-identically on
        // every application (no behavior change) and matches the non-hygienic
        // `apply` output.
        let mut registry = MacroRegistry::new();
        let kind = SyntaxKind::app("myMacro");
        let def = MacroDef::new(
        ...
        assert!(
            !def.replacement.has_fresh_markers(),
            "static template carries no fresh markers"
        );
        registry.register(def);

        let input = Syntax::node(kind, vec![Syntax::ident("arg")]);
        let mut expander = HygienicExpander::new(&registry);
        let r1 = expander.expand(input.clone()).expect("expand once");
        let r2 = expander.expand(input).expect("expand twice");
        assert_eq!(
            r1.pretty(),
            r2.pretty(),
            "static template must expand identically every time"
        );
        assert_eq!(r1.child(1).unwrap().as_ident(), Some("arg"));
    }
