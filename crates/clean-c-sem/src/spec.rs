...
//!
//! ```
//! use clean_c_sem::spec::{FuncSpec, Spec};
//! use clean_c_sem::expr::BinOp;
//!
//! // Specifying:
//! //   /*@ requires n >= 0;
//! //       ensures \result >= 0;
...
//!
//! let spec = FuncSpec {
//!     requires: vec![Spec::ge(Spec::var("n"), Spec::int(0))],
//!     ensures: vec![
    ...
    pub fn and(specs: Vec<Spec>) -> Self {
        if specs.is_empty() {
            ...
            specs
                .into_iter()
                .next()
                .expect("invariant: specs.len() == 1 checked above")
        } else {
            Spec::And(specs)
        }
    }
...
mod tests {
    ...
    fn test_loop_spec() {
        ...
        assert!(spec.variant.is_some(), "loop spec should have a variant");
    }
