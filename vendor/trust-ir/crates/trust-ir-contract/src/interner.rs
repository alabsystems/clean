...
/// - Zero-cost clone
///
/// Resolve back to the string via [`Symbol::as_str()`] or [`std::fmt::Display`].
...
// Enable `assert_eq!(symbol, "str")` and `symbol == "str"` comparisons.
...
// Enable Symbol to convert back to String via From/Into
...
// Enable `symbol.as_bytes()` for hashing/caching paths
// Delegate common str methods for ergonomic use
impl Symbol {
...
mod tests {
    ...
    // Tests for backward-compatible conversions added for interning extension
